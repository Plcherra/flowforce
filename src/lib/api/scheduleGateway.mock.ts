...
import type {
  CreateVendorVisitPayload,
  ScheduleGateway,
} from "./scheduleGateway";
...
const buildVendorVisitRow = (
  payload: CreateVendorVisitPayload,
  linkedEventId: string,
): Tables<"vendor_visits"> => {
  const timestamp = now();
  const companyId =
    payload.vendor.company_id ?? payload.calendar.company_id ?? null;
  if (!companyId) {
  ...
  return {
    ...
    start_time:
      payload.vendor.start_time ?? payload.calendar.start_time ?? timestamp,
