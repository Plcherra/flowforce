...
function deriveRiskLevel(
  ...
  else if (predictedBacklog > Math.max(3, baseline.tasks.backlog * 0.6))
    score += 1;
...
function buildSchedulingAction(
  ...
  if (
    predictedCoverage >= 0.97 &&
    predictedOpenShifts <= Math.max(2, baseline.scheduling.openShifts * 0.4)
  ) {
    return null;
  }

  const impact: ImpactLevel = predictedCoverage < 0.92 ? "high" : "medium";
  const confidenceSeed = clamp(
    1 - baseline.scheduling.overtimeRisk + adjustments.staffingChangePct / 50,
    0.35,
    0.85,
  );
    ...
    metrics: [
      {
        ...
        value: `${(predictedCoverage * 100).toFixed(1)}%`,
      },
      {
        label: "Unfilled Shifts",
        value: `${Math.max(0, Math.round(predictedOpenShifts))}`,
      },
    ],
  };
}
...
function buildTaskAction(
  ...
  const impact: ImpactLevel =
    predictedBacklog > baseline.tasks.backlog ? "high" : "medium";
  const dueDate = addWeeks(
    new Date(),
    Math.max(1, Math.min(6, adjustments.timelineWeeks - 1)),
  );
  ...
  return {
    ...
    impact,
    confidence: Number(
      clamp(0.6 + adjustments.taskAutomationPct / 200, 0.4, 0.9).toFixed(2),
    ),
    suggestedDueDate: dueDate.toISOString(),
    metrics: [
      ...
      {
        label: "Backlog Δ",
        value: `${backlogDelta >= 0 ? "+" : ""}${Math.round(backlogDelta)}`,
      },
    ],
  };
}
...
function buildGoalAction(
  ...
  const dueDate = addWeeks(
    new Date(),
    Math.max(1, Math.min(4, adjustments.timelineWeeks - 2)),
  );
  ...
  return {
    ...
    impact,
    confidence: Number(
      clamp(0.55 + adjustments.goalFocusPct / 150, 0.45, 0.85).toFixed(2),
    ),
    suggestedDueDate: dueDate.toISOString(),
    metrics: [
...
function buildRevenueAction(
  ...
  if (
    predictedRevenue >= baseline.revenue.forecastNext30 &&
    predictedMarginRate >= baseline.revenue.marginRate
  ) {
    return null;
  }
  ...
  const dueDate = addWeeks(
    new Date(),
    Math.max(2, Math.min(8, adjustments.timelineWeeks)),
  );
  const impact: ImpactLevel =
    predictedRevenue < baseline.revenue.forecastNext30 * 0.95 ||
    predictedMarginRate < baseline.revenue.marginRate - 0.05
      ? "high"
      : "medium";
  ...
  return {
    ...
    impact,
    confidence: Number(
      clamp(0.5 + adjustments.revenueChangePct / 120, 0.4, 0.8).toFixed(2),
    ),
    suggestedDueDate: dueDate.toISOString(),
    metrics: [
      {
      ...
      },
      {
        ...
        value: `${(predictedMarginRate * 100).toFixed(1)}%`,
      },
    ],
  };
}
...
export function calculateScenarioOutcome(
  ...
  const predictedOpenShifts = Math.max(
    ...
    baseline.scheduling.openShifts *
      (1 - (adjustments.staffingChangePct * 0.75) / 100),
  );

  const backlogBase = baseline.tasks.backlog;
  const backlogReduction =
    backlogBase * ((adjustments.taskAutomationPct * 0.65) / 100);
  const predictedBacklog = clamp(
    backlogBase - backlogReduction,
    0,
    backlogBase + 20,
  );
  ...
  const goalBoost =
    adjustments.goalFocusPct * 0.7 + adjustments.taskAutomationPct * 0.1;
  const coveragePenalty = (coverageBase - predictedCoverage) * 45;
  const predictedGoalConfidence = clamp(
    goalConfidenceBase + goalBoost - coveragePenalty,
    0,
    100,
  );

  const revenueBase = baseline.revenue.forecastNext30;
  const revenueStaffingLift =
    revenueBase * ((adjustments.staffingChangePct * 0.12) / 100);
  const revenueAutomationLift =
    revenueBase * ((adjustments.taskAutomationPct * 0.08) / 100);
  const revenueDelta =
    revenueBase * ((adjustments.revenueChangePct * 0.85) / 100);
  const predictedRevenue = Math.max(
    revenueBase + revenueStaffingLift + revenueAutomationLift + revenueDelta,
    0,
  );

  const operatingCostBase = baseline.revenue.operatingCost30;
  const overtimeSavings =
    operatingCostBase * ((adjustments.overtimeReductionPct * 0.4) / 100);
  const automationSavings =
    operatingCostBase * ((adjustments.taskAutomationPct * 0.12) / 100);
  ...
  const predictedMarginRate =
    predictedRevenue > 0
      ? clamp(
          (predictedRevenue - predictedOperatingCost) / predictedRevenue,
          0,
          0.82,
        )
      : 0;
  ...
  const schedulingAction = buildSchedulingAction(
    predictedCoverage,
    predictedOpenShifts,
    baseline,
    adjustments,
  ...
  const goalAction = buildGoalAction(
    predictedGoalConfidence,
    baseline,
    adjustments,
  );
  ...
  const revenueAction = buildRevenueAction(
    predictedRevenue,
    predictedMarginRate,
    baseline,
    adjustments,
