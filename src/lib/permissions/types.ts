 ...
 */
export function assertValidPermissionKey(
  key: string,
): asserts key is PermissionKey {
  if (!isValidPermissionKey(key)) {
    throw new Error(
      `Invalid permission key: ${key}. Valid keys: ${PERMISSION_KEYS.join(", ")}`,
    );
  }
}
...
export const PERMISSION_CATEGORIES = {
  ...
  ADMIN: PERMISSION_KEYS.filter(
    (key) => key.includes("admin.") || key.includes("manage"),
  ),
} as const;
