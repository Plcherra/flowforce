...
import { allowEdit, getLockStateForWeek, computeAutoLockThreshold, type LockEngineDeps } from '@/availability/lockEngine';
...
describe('availability lock engine', () => {
  ...
  it('handles auto mode before and after threshold', async () => {
    ...
    const before = await getLockStateForWeek({ orgId: 'org-1', weekStart, deps });
    expect(before).toBe('open');

    vi.setSystemTime(new Date(threshold.getTime() + 60 * 60 * 1000));
    const after = await getLockStateForWeek({ orgId: 'org-1', weekStart, deps });
  ...
  it('auto mode allows exceptions after threshold', async () => {
    ...
    const state = await getLockStateForWeek({ orgId: 'org-1', weekStart, deps });
