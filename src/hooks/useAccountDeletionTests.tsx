  ...
  const runDeletionTests = async () => {
    if (!testEmail || !testPassword) {
      toast({
        ...
        description: "Please provide test email and password for a deleted account.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (loginError || !loginData.user) {
        setTestResults(prev => ({ ...prev, loginAttemptAfterDeletion: 'success' }));
      } else {
        setTestResults(prev => ({ ...prev, loginAttemptAfterDeletion: 'failed' }));
      ...
      toast({
        ...
        description: "Check the results below to verify account deletion is working properly.",
      });
