...
export type FeedSource = 'tasks' | 'goals' | 'reminders' | 'notifications' | 'scheduling' | 'expenses';
...
export function useAIActionsFeed() {
  ...
  const items = useMemo<AIActionsFeedItem[]>(() => {
    ...
    const isTaskClosed = (status?: string | null) => status === 'completed' || status === 'cancelled';

    const overdueTasks = tasks.filter(
      (task) => task.due_date && new Date(task.due_date) < now && !isTaskClosed(task.status),
    );
    ...
    if (atRiskGoals.length > 0) {
      const overdueGoals = atRiskGoals.filter(
        (goal) => goal.target_completion_date && new Date(goal.target_completion_date) < now,
      );
      const severity: FeedSeverity = overdueGoals.length > 0 ? 'high' : 'medium';
    ...
    if (overdueReminders.length > 0) {
      ...
      const imminentReminder = pendingReminders
        .filter((reminder) => reminder.remind_at && differenceInMinutes(new Date(reminder.remind_at), now) <= 60)
        .sort((a, b) => new Date(a.remind_at ?? '').getTime() - new Date(b.remind_at ?? '').getTime())[0];
      ...
      if (imminentReminder) {
        ...
        itemsAccumulator.push({
          ...
          description: relative ? `Scheduled ${relative}. Confirm next steps or defer.` : 'Reminder coming due shortly.',
          severity: 'low',
          source: 'reminders',
          details: [`Priority ${imminentReminder.priority}`, `Channel: ${(imminentReminder.type ?? 'general')}`],
          actions: [
            {
              type: 'complete_reminder',
              label: 'Mark done',
    ...
    const shiftCoverageGaps = shifts
      .map((shift) => {
        ...
        const assigned = (shift.assignments ?? []).filter((assignment) => assignment?.status !== 'cancelled').length;
        const gap = required - assigned;
        if (gap <= 0) return null;
        return { shift, gap, shiftStart };
      })
      .filter(
        (entry): entry is { shift: (typeof shifts)[number]; gap: number; shiftStart: Date } => Boolean(entry),
      );

    if (shiftCoverageGaps.length > 0) {
      const topGap = shiftCoverageGaps.sort((a, b) => (b?.gap ?? 0) - (a?.gap ?? 0))[0]!;
      ...
      itemsAccumulator.push({
        ...
        severity: shiftCoverageGaps.length > 1 || topGap.gap > 1 ? 'high' : 'medium',
        source: 'scheduling',
    ...
    const previousWeekExpenses = expensesByWindow(previousWeekStart, lastWeekStart);

    const sumExpenses = (list: Expense[]) => list.reduce((sum, expense) => sum + (expense.amount ?? 0), 0);
    ...
    const pctChange =
      previousWeekTotal > 0 ? Math.round(((lastWeekTotal - previousWeekTotal) / previousWeekTotal) * 100) : null;

    const significantSpike =
      (pctChange !== null && pctChange >= 25 && lastWeekTotal > 0) || (previousWeekTotal === 0 && lastWeekTotal >= 500);

    if (significantSpike) {
      const categoryTotals = lastWeekExpenses.reduce<Record<string, number>>((acc, expense) => {
      ...
      const categoryLabel = topCategory ? `${topCategory[0]} ($${topCategory[1].toFixed(0)})` : 'Mixed categories';
