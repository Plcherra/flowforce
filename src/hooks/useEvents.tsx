...
const DEFAULT_EVENT_COLORS: Record<
  "meeting" | "event" | "vendor_visit",
  string
> = {
  meeting: "#0ea5e9",
  event: "#6366f1",
...
const readStoredEvents = (key: string): AppEvent[] | undefined => {
  ...
  try {
    ...
    const mapped = parsed.map((entry) => ({
      ...
      source:
        entry.source ?? (entry.type === "vendor_visit" ? "vendor" : "calendar"),
    }));
...
const persistStoredEvents = (key: string, events: AppEvent[]) => {
  ...
  try {
    const payload = events.map(({ persisted, source, ...rest }) => ({
      ...rest,
      persisted,
      source,
    }));
    ...
    logger.error("Failed to persist events locally", {
      error,
      tags: ["error"],
    });
  }
...
const mapRowToEvent = (row: CalendarEventRow): AppEvent => {
  ...
  return {
    ...
    related_shift_ids: Array.isArray(row.related_shift_ids)
      ? row.related_shift_ids
      : [],
    ...
    persisted: true,
    source:
      metadata?.source ?? (type === "vendor_visit" ? "vendor" : "calendar"),
  };
};
...
const syncEventParticipants = async (
  ...
  try {
    ...
    logger.warn("Failed to sync event participants", {
      error,
      tags: ["warning"],
    });
  }
...
const syncEventShiftLinks = async (
  ...
  try {
    const uniqueShiftIds = Array.from(
      new Set((shiftIds ?? []).filter(Boolean)),
    );
    ...
    logger.warn("Failed to sync event shift links", {
      error,
      tags: ["warning"],
    });
  }
...
const toUpdatePayload = (updates: Partial<AppEvent>) => {
  ...
  if (updates.related_shift_ids !== undefined)
    payload.related_shift_ids = updates.related_shift_ids ?? [];
...
export function useEvents() {
  ...
  const [localEvents, setLocalEvents] = useState<AppEvent[]>(
    () => readStoredEvents(defaultStorageKey) ?? [],
  );
  ...
  const eventsQuery = useQuery({
    queryKey: companyId
      ? queryKeys.calendarEventsList(companyId)
  ...
  const invalidateCompanyEvents = useCallback(() => {
    ...
    void queryClient.invalidateQueries({
      queryKey: queryKeys.calendarEventsCompany(companyId),
    });
  ...
  >({
    mutationFn: async (payload) => {
      ...
      const insertPayload = toInsertPayload(
        normalized as AppEvent,
        companyId,
        user?.id ?? null,
        "calendar",
      );
      const eventRow = await scheduleGateway
        .createEvent(insertPayload)
        .catch((error) => {
          const normalizedError = normalizeCalendarError(error);
          throw new CalendarError(normalizedError.message, {
  ...
  const createVendorVisitMutation = useMutation<
    AppEvent,
    CalendarError,
  ...
  >({
    mutationFn: async (payload) => {
      ...
      const vendorDetails = normalized.vendor as
        | Record<string, unknown>
        | undefined;
      const vendorPayload = {
        company_id: companyId,
        vendor_name:
          normalized.vendor?.name ?? normalized.title ?? "Vendor Visit",
  ...
  const updateEventMutation = useMutation<
    AppEvent,
    CalendarError,
    { id: string; updates: Partial<AppEvent> }
  >({
    mutationFn: async ({ id, updates }) => {
      if (!companyId) throw new Error("Company context is not available");
      const row = await calendarEventsRepository
        .updateEvent(id, toUpdatePayload(updates))
        .catch((error) => {
          const normalizedError = normalizeCalendarError(error);
          throw new CalendarError(normalizedError.message, {
            code: normalizedError.code,
  ...
  const getEventsForShift = useCallback(
    (shiftId: string) =>
      events.filter((event) =>
        (event.related_shift_ids ?? []).includes(shiftId),
      ),
    [events],
  );
  ...
  const linkVisitToShifts = useCallback(
    async (eventId: string, shiftIds: string[]) => {
      if (!companyId) {
        updateLocalEvents((previous) =>
          sortEvents(
            previous.map((entry) =>
              entry.id === eventId
                ? { ...entry, related_shift_ids: shiftIds }
                : entry,
            ),
          ),
        );
      ...
      await updateEventMutation.mutateAsync({
        id: eventId,
        updates: { related_shift_ids: shiftIds },
      });
    },
    [companyId, updateEventMutation, updateLocalEvents],
  );
