...
export function useAnalytics() {
  return useQuery({
    ...
    queryFn: async (): Promise<AnalyticsData> => {
      ...
      const monthlyTrends = [
        { month: 'Jan', employees: totalEmployees || 0, tasks: completedTasks || 0, timeOff: pendingTimeOffRequests || 0 },
        { month: 'Feb', employees: (totalEmployees || 0) + 2, tasks: (completedTasks || 0) + 5, timeOff: (pendingTimeOffRequests || 0) + 1 },
        { month: 'Mar', employees: (totalEmployees || 0) + 1, tasks: (completedTasks || 0) + 8, timeOff: (pendingTimeOffRequests || 0) + 3 },
      ];
      ...
      const employeeData = await Promise.all(
        profiles.map(async (profile) => {
          ...
          return {
            ...
            completion_rate: totalTasks && totalTasks > 0 ? 
              ((completedTasks || 0) / totalTasks) * 100 : 0
          };
        })
      );
