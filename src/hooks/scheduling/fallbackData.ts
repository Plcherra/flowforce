...
const createShift = (
  ...
  const start_time = createDateTimeString(base, config.dayOffset, config.startTime);
  const end_time = createDateTimeString(base, config.dayOffset, config.endTime);

  const assignments: AssignmentWithUser[] = (config.assignments ?? []).map((assignment, index) => ({
...
export function buildSchedulingFallbackData(params: BuildFallbackParams = {}): SchedulingFallbackData {
  ...
  const shiftResults = shiftConfigs.map((config) => createShift(base, config, nowIso));
