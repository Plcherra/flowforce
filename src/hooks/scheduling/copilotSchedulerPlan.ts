...
const initialiseEmployees = (
  ...
  const summary: ScheduleSummary = { totalHours: 0, hoursByEmployee: {}, hoursByStore: {} };

  employees.forEach((employee) => {
    const baselineHours = existingHours?.get(employee.id) ?? 0;
    const baselineStoreHours = cloneHoursByStore(existingHoursByStore?.get(employee.id));
    map.set(employee.id, { employee, hours: baselineHours, hoursByStore: baselineStoreHours });
...
export function generateDraftSchedulePlan({
  ...
  const { map: employeeStateMap, summary } = initialiseEmployees(employees, existingHours, existingHoursByStore);
  ...
  for (const template of sortTemplates(templates)) {
    ...
    if (requiredSlots <= 0 && template.requiredCount > 0) requiredSlots = template.requiredCount;
    ...
    while (assignedCount < requiredSlots) {
      const eligible = Array.from(employeeStateMap.values()).filter((state) => {
        ...
        if (!isEmployeeAvailable(state.employee, scheduleDate, startDate, endDate)) return false;
        return true;
      });
      ...
      const ordered = isSupervisorRole(template.role)
        ...
        : [...eligible].sort((a, b) => {
            const storePenaltyA = a.employee.homeStore && a.employee.homeStore !== template.location ? 2 : 0;
            const storePenaltyB = b.employee.homeStore && b.employee.homeStore !== template.location ? 2 : 0;
            return a.hours + (a.hoursByStore[template.location] ?? 0) * 0.75 + storePenaltyA -
              (b.hours + (b.hoursByStore[template.location] ?? 0) * 0.75 + storePenaltyB);
          });
      ...
      chosen.hoursByStore[template.location] = (chosen.hoursByStore[template.location] ?? 0) + shiftHours;
      ...
      summary.hoursByStore[chosen.employee.id] = cloneHoursByStore(chosen.hoursByStore);

      if (isSupervisorRole(template.role)) recordSupervisorAssignment(supervisorLedger, chosen.employee.id, template.location);

      const scheduleDateIso = scheduleDate.toISOString().split('T')[0] ?? '';
      draftShifts.push({
        dedupeKey: buildDedupeKey(template.id, scheduleDateIso, chosen.employee.id, assignedCount),
    ...
    if (assignedCount < requiredSlots) {
      ...
      const donor = draftShifts
        ...
        .find(
          (shift) =>
            shift.role === template.role && shift.location !== template.location && shift.employeeId !== null,
        );

      const crossStoreCandidate = Array.from(employeeStateMap.values()).find((state) => {
        if (!employeeMatchesRole(state.employee, template.role)) return false;
        if (state.employee.homeStore && state.employee.homeStore === template.location) return false;
        if (!isEmployeeAvailable(state.employee, scheduleDate, startDate, endDate)) return false;
...
export const computeExistingHours = (
  shifts: Array<{ employee_id: string | null; start_time: string; end_time: string; location?: string | null }>,
) => {
