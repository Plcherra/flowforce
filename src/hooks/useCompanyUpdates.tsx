...
import { companyUpdatesRepository, type CompanyUpdateRow, type ReactionRow } from '@/repositories/companyUpdatesRepository';
...
export function useCompanyUpdates(options?: UseCompanyUpdateOptions) {
  ...
  const updatesQuery = useQuery({
    queryKey: [UPDATES_QUERY_KEY, companyId, page, pageSize, statusFilter ?? 'all', searchTerm.toLowerCase()],
  ...
  const updateIds = useMemo(() => (updatesQuery.data?.records ?? []).map((record) => record.id), [updatesQuery.data?.records]);
