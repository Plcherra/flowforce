  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        logger.error("Unexpected profiles query error", {
          error,
          tags: ["error"],
        });
        return [];
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        logger.error("Unexpected roles query error", {
          error,
          tags: ["error"],
        });
        return [];
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["employee-access", variables.employeeId],
      });
      toast({
        title: "Role updated",
...
function normalizeRoleName(roleName: string) {
  ...
  const allowed: Array<
    "admin" | "manager" | "employee" | "staff" | "supervisor" | "owner"
  > = ["admin", "manager", "employee", "staff", "supervisor", "owner"];
...
async function resolveCompanyId(userId: string, fallback: string | null) {
  try {
    ...
    if (error) {
      logger.error("Failed to resolve company id for profile", {
        error,
        tags: ["error"],
      });
      return fallback;
    ...
    logger.error("Unexpected error resolving company id", {
      error,
      tags: ["error"],
    });
    return fallback;
