...
import { learningInvalidators, learningKeyHelpers, learningKeys } from '@/features/learning/hooks/queryKeys';
import type {
...
export function useLearningCenter() {
  ...
  const enrollmentIdsKey = useMemo(() => enrollmentList.map((enrollment) => enrollment.id).sort().join('|'), [enrollmentList]);
  ...
  const progressQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      entries.forEach(({ enrollmentId, events, snapshots, eventCursor, snapshotCursor }) => {
  ...
  const adminEnrollments = trainingAdmin ? adminEnrollmentsQuery.data ?? [] : [];
  const skillSnapshot = skillSnapshotQuery.data ?? null;
  const trainingInsights = trainingAdmin ? trainingInsightsQuery.data ?? null : null;
  ...
  }, [catalog, enrollmentList, metrics, skillSnapshot?.role, skillSnapshot?.xp, profile?.role]);
  ...
  const loading =
    ...
    (trainingAdmin && (adminEnrollmentsQuery.isLoading || metricsQuery.isLoading || trainingInsightsQuery.isLoading));

  const error =
    catalogQuery.error?.message ??
    enrollmentsQuery.error?.message ??
    progressQuery.error?.message ??
    (trainingAdmin ? adminEnrollmentsQuery.error?.message ?? metricsQuery.error?.message ?? trainingInsightsQuery.error?.message : null) ??
    null;

  const createCourseMutation = useMutation({
    mutationFn: async (payload: CourseCreationPayload) => {
      if (!user?.id || !companyId) {
        throw new Error(!user?.id ? 'Please sign in to create training courses.' : 'Company context missing for learning data.');
      }
    ...
    onError: (err: Error) => {
      toast({
        ...
        description: err.message ?? 'Please try again or contact an administrator.',
        variant: 'destructive',
      });
    },
  });
  ...
  const handleCreateCourse = useCallback(
    (payload: CourseCreationPayload) => createCourseMutation.mutateAsync(payload),
  ...
  const handleEnroll = useCallback(
    ...
    [profileId, companyId, queryClient, companyKey, profileKey, enrollmentIdsKey],
  );

  const handleModuleCompletion = useCallback(
    async (enrollmentId: string, moduleIndex: number) => {
      const enrollment = enrollmentList.find((item) => item.id === enrollmentId);
...
async function fetchScopedSkillSnapshot(userId: string, companyId: string): Promise<SkillSnapshot | null> {
...
async function fetchAdminEnrollments(limit = ADMIN_ENROLLMENT_LIMIT): Promise<LearningEnrollment[]> {
  const { data, error } = await supabase.functions.invoke('learning-admin-enrollments', {
    body: { limit },
  });
