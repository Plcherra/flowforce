...
describe("buildReviewAutomationPlan", () => {
  it("creates coaching task and reminder when employees lack recent reviews", () => {
    ...
    const reminderEmployees = plan.reminderTemplates.map(
      (template) => template.employeeId,
    );
    expect(reminderEmployees).toEqual(
      expect.arrayContaining(["emp-2", "emp-3"]),
    );
  ...
  it("creates task and reminder for overdue reviews", () => {
    ...
    const overdueTask = plan.taskTemplates.find(
      (task) => task.employeeId === "emp-2",
    );
    expect(overdueTask).toBeDefined();
    expect(overdueTask?.priority).toBe("urgent");
    expect(overdueTask?.dueInDays).toBe(3);

    const overdueReminder = plan.reminderTemplates.find(
      (reminder) => reminder.employeeId === "emp-2",
    );
  ...
  it("skips automation when active tasks or reminders already exist", () => {
    ...
    const reminderEmployeeIds = plan.reminderTemplates.map(
      (reminder) => reminder.employeeId,
    );
