...
export async function fetchWorkflowSnapshot(rulebook: ScheduleRulebook, workflowId: string): Promise<WorkflowSnapshot> {
  if (!workflowId) {
    return {};
  }
  ...
  (data || []).forEach((workflowStep) => {
    ...
    const stepEntry: WorkflowStepSnapshot = snapshot[stepSlug] || {
      ...
      state: (workflowStep.state ?? 'not_started') as WorkflowStepSnapshot['state'],
      criteria: {},
    };

    stepEntry.workflowStepId = workflowStep.id;
    stepEntry.state = (workflowStep.state ?? 'not_started') as WorkflowStepSnapshot['state'];

    const criteriaRecords = workflowStep.schedule_workflow_criteria || [];

    criteriaRecords.forEach((criterionRecord: any) => {
      const criterionSlug = criterionRecord.schedule_rulebook_step_criteria?.slug;
      if (!criterionSlug) {
        return;
      }
      ...
      stepEntry.criteria[criterionSlug] = {
        ...
        status: (criterionRecord.status ?? 'pending') as WorkflowCriterionSnapshot['status'],
...
export async function upsertWorkflowCriterionState(params: {
  ...
  const payload: Record<string, unknown> = {
    ...
    evidence_value: params.value !== undefined ? JSON.stringify({ value: params.value }) : null,
  };
  ...
  const { data, error } = await supabase
    ...
    .select('id, workflow_step_id, rulebook_criterion_id, status, evidence_value, approved_by, approved_at')
    .single();
...
async function ensureWorkflowSteps(rulebook: ScheduleRulebook, workflowId: string) {
  ...
  const existing = new Set((data || []).map((row: any) => row.rulebook_step_id));
...
function parseEvidenceValue(value: unknown): number | string | boolean | undefined {
  if (!value) {
    return undefined;
  }
...
export function deriveDefaultValue(criterion: StepCriterion): number | string | boolean | undefined {
