...
vi.mock('@/integrations/supabase/client', () => {
  ...
  const getFilteredRows = (table: string, state: QueryState): any[] => {
    ...
    return rows.filter((row) =>
      state.eqCalls.every(({ column, value }) => resolveColumn(row, column) === value),
    );
  };
...
describe('learningService tenant isolation', () => {
  ...
  it('restricts fetchLearningCatalog to the active company', async () => {
    ...
    expect(catalog.map((course) => course.id)).toEqual(['course-1', 'course-2']);
    expect(catalog.every((course) => course.modules.every((module) => module.courseId === course.id))).toBe(true);
    ...
    const metricsState = supabaseMock.__getTableState('learning_course_metrics');

    expect(courseState?.eqCalls).toContainEqual({ column: 'company_id', value: 'company-1' });
    expect(moduleState?.eqCalls).toContainEqual({ column: 'company_id', value: 'company-1' });
    expect(metricsState?.eqCalls).toContainEqual({ column: 'company_id', value: 'company-1' });
  });
