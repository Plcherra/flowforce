...
const makeEmployee = (
  overrides: Partial<SchedulerEmployee>,
): SchedulerEmployee => ({
...
const makeTemplate = (
  overrides: Partial<CoverageTemplatePlan>,
): CoverageTemplatePlan => ({
  id: overrides.id ?? randomUUID(),
  companyId: "company-1",
...
describe("generateDraftSchedulePlan", () => {
  it("calculates total hours across assigned shifts", () => {
    const employees = [
      makeEmployee({ id: "emp-a" }),
      makeEmployee({ id: "emp-b" }),
    ];
    const templates = [
      makeTemplate({ id: "tpl-1", dayOfWeek: 1, requiredCount: 2 }),
    ];
    const forecastMap = buildForecastMap([["tpl-1", 2]]);
  ...
  it("respects availability and 38-hour per store cap", () => {
    ...
    const thursdayGap = plan.coverageGaps.find(
      (gap) => gap.templateId === "tpl-thu",
    );
    expect(thursdayGap).toBeDefined();
  });

  it("proposes valid swap suggestions for cross-store coverage gaps", () => {
    const supervisorStore2Primary = makeEmployee({
      id: "sup-s2-primary",
      homeStore: "Store 2",
      weeklyMaxHours: 8,
    });
    const supervisorStore2Flex = makeEmployee({
      id: "sup-s2-flex",
      homeStore: "Store 2",
      weeklyMaxHours: 8,
    });
    const supervisorStore1Unavailable = makeEmployee({
      id: "sup-s1-unavailable",
      homeStore: "Store 1",
      availability: { mon: [] },
      weeklyMaxHours: 0,
    });
    ...
    const plan = generateDraftSchedulePlan({
      employees: [
        supervisorStore2Primary,
        supervisorStore2Flex,
        supervisorStore1Unavailable,
      ],
      templates,
      weekStart,
      weekEnd,
...
describe("rotateSupervisors", () => {
  it("balances supervisors across stores", () => {
    ...
    const ledger = new Map<
      string,
      { assignmentsByStore: Record<string, number>; totalAssignments: number }
    >();
