...
import type {
  ...
  ComplianceRules,
} from "@/data/scheduling/locationRuleSets";
...
const DEFAULT_AVAILABILITY: CopilotEmployeeSeed["availability"] =
...
const resolveArea = (
  ...
  const metaArea =
    typeof metaAreaRaw === "string" ? metaAreaRaw.toUpperCase() : null;
  if (metaArea === "BOH" || metaArea === "FOH") {
    return metaArea;
  }
  const roleLabel = role?.toLowerCase() ?? "";
  if (
    roleLabel.includes("cook") ||
    roleLabel.includes("boh") ||
    roleLabel.includes("kitchen")
  ) {
    return "BOH";
  }
  return "FOH";
...
async function buildRuleSetFromDatabase(
  ...
  templateRows
    ...
    .forEach((row) => {
      const weekdayIndex = Number.isInteger(row.day_of_week)
      ...
      const slot: CoverageSlotTemplate = {
        ...
        tags: Array.isArray((metadata as { tags?: string[] }).tags)
      ...
      if (
        !detectedTimezone &&
        typeof (metadata as { timezone?: string }).timezone === "string"
      ) {
  ...
  const employeeSeeds: CopilotEmployeeSeed[] = employeeRows.map((row) => {
    const metadata = (row.metadata as Record<string, unknown> | null) ?? {};
    const secondaryRoles = Array.isArray(row.secondary_roles)
      ? (row.secondary_roles as string[])
      : [];
    const metaDisplayName = (metadata as { display_name?: string })
      .display_name;
    const metaLocations = (metadata as { locations?: string[] }).locations;
    const preferredDays = (metadata as { preferred_days_off?: number[] })
      .preferred_days_off;
    ...
    return {
      ...
      locationIds:
        Array.isArray(metaLocations) && metaLocations.length > 0
          ? metaLocations
          : [row.home_store ?? locationId],
      area: resolveArea(metadata, row.role),
...
function buildShiftSlots(
  ...
  weekStart: Date,
): { slots: ShiftSlot[]; meta: Map<string, SlotMeta> } {
...
function aggregateCoverage(
  ...
  const coverageByArea: AutoScheduleSummary["coverageByArea"] = {
    FOH: { required: 0, assigned: 0, ratio: 0 },
    BOH: { required: 0, assigned: 0, ratio: 0 },
  };
  ...
  slots.forEach((slot) => {
    ...
    const dayRecord = coverageByDay[dayKey] ?? {
      required: 0,
      assigned: 0,
      ratio: 0,
    };
    ...
    coverageByArea[slot.area] = coverageByArea[slot.area] ?? {
      required: 0,
      assigned: 0,
      ratio: 0,
    };
  ...
  Object.values(coverageByDay).forEach((record) => {
    record.ratio =
      record.required > 0
        ? Number((record.assigned / record.required).toFixed(2))
        : 1;
  });
...
function evaluateCompliance(
  ...
  assignmentsByEmployee.forEach((employeeAssignments, employeeId) => {
    ...
    const totalHours = sorted.reduce(
      (total, assignment) =>
        total +
        differenceInHours(
          buildDateWithTime(new Date(assignment.date), assignment.end),
          buildDateWithTime(new Date(assignment.date), assignment.start),
        ),
      0,
    );
    ...
    for (let i = 0; i < sorted.length - 1; i += 1) {
      ...
      if (rest < compliance.minRestHours) {
        warnings.push({
          ...
          fixes: [
            {
              label: "Move next shift",
              payload: { employeeId, slotId: next.slotId },
            },
          ],
        });
      }
    ...
    const assignedDays = sorted.map((assignment) =>
      buildDateWithTime(new Date(assignment.date), assignment.start).getDay(),
    );
    let currentStreak = 1;
    let maxStreak = 1;
  ...
  assignments.forEach((assignment) => {
    ...
    if (
      template.isCloser &&
      !employee.availability.some((entry) => entry.canClose)
    ) {
      warnings.push({
...
function convertEngineWarnings(
  engineResult: GenerateScheduleResult,
): CopilotDraftWarning[] {
...
function buildSummary(
  ...
  const { coverageByDay, coverageByArea } = aggregateCoverage(
    slots,
    assignments,
  );
...
export async function runCopilotAutoSchedule(
  ...
  const employees = ruleset.employeeSeeds
    .map(seedToProfile)
    .filter((employee) => employee.locationIds.includes(ruleset.id));

  const policyEngine = new PolicyEngine();
  const engineResult = await generateSchedule(policyEngine, {
    slots,
    employees,
  });

  const complianceWarnings = evaluateCompliance(
    ruleset,
    engineResult.assignments,
    meta,
    employees,
  );
  ...
  const assignmentsBySlot = new Map(
    engineResult.assignments.map((assignment) => [
      assignment.slotId,
      assignment,
    ]),
  );
...
export async function describeAvailableRuleSets(companyId: string) {
  const [locations, employees] = await Promise.all([
    listCoverageLocations(companyId),
