import { getScheduleRulebook } from "../scheduleRulebookService";
import type {
  ScheduleRulebook,
  RulebookStep,
  RulebookConstraint,
...
export class ScheduleGuardrailEngine {
  ...
  public evaluate(context: GuardrailContext): GuardrailResult {
    ...
    if (step) {
      ...
      if (!roleAllowed) {
        return this.blocked(
          `Role ${context.actorRole} is not permitted to perform ${context.action} on step ${step.title}.`,
          {
  ...
  private evaluateCriterion(
    ...
    if (
      criterion.evidenceType === "numeric" &&
    ...
    if (
      criterion.approverRole &&
      context.pendingApprovals &&
      !context.pendingApprovals[criterion.id]
    ) {
      return {
        rulebookId: this.rulebook.id,
        stepId: context.stepId,
  ...
  private evaluateConstraints(
    ...
    const violations = this.rulebook.constraints
      .filter((constraint) =>
        this.constraintApplies(constraint, context.action),
      )
  ...
  private buildConstraintViolation(
    constraint: RulebookConstraint,
  ): GuardrailFailure {
    return {
      rulebookId: this.rulebook.id,
