...
export async function fetchRulebookFromSupabase(
  identifier: string,
): Promise<ScheduleRulebook | null> {
...
function mapRulebook(rulebook: DbRulebook): ScheduleRulebook {
  const steps: RulebookStep[] = [...(rulebook.schedule_rulebook_steps || [])]
    ...
    .map((step) => ({
      ...
      followUps: (step.schedule_rulebook_step_followups || []).map(
        (followUp) => ({
          id: followUp.id,
  ...
  const constraints: RulebookConstraint[] = (
    rulebook.schedule_rulebook_constraints || []
  ).map((constraint) => ({
    id: constraint.slug,
