...
export async function fetchSchedulingWeek(params: {
  ...
  const shiftWithAssignments: ShiftWithAssignments[] = schedules.map(
    (shift) => ({
      ...shift,
      assignments: assignmentsWithUsers.filter(
        (assignment) => assignment.schedule_id === shift.id,
      ),
    }),
  );
...
export async function upsertVendorEvent(
...
export async function deleteVendorEventRecord(
  id: string,
  companyId?: string | null,
): Promise<void> {
...
export async function insertSchedules(
  payloads: TablesInsert<"schedules">[],
): Promise<Tables<"schedules">[]> {
  ...
  if (invalidPayloads.length > 0) {
    throw new Error(
      `All schedules must have company_id. ${invalidPayloads.length} payload(s) missing company_id.`,
    );
  }
...
export async function insertScheduleAssignments(
  payloads: TablesInsert<"schedule_assignments">[],
): Promise<void> {
  if (!payloads.length) return;
  const { error } = await supabase
    .from("schedule_assignments")
...
export async function deleteCopilotDrafts(params: {
  ...
  const { companyId, locationName, locationId, weekStartIso, weekEndIso } =
    params;
  const { data, error } = await supabase
    .from("schedules")
    ...
    data
      ?.filter((row) => {
        const requirements = row.requirements as {
          copilot?: { locationId?: string; weekStart?: string };
        } | null;
        const copilot = requirements?.copilot;
        return (
          copilot?.locationId === locationId &&
          copilot?.weekStart === weekStartIso
        );
      })
      .map((row) => row.id) ?? [];
