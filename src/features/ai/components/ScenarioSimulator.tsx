...
export default function ScenarioSimulator({
  ...
  const [adjustments, setAdjustments] =
    useState<ScenarioAdjustments>(DEFAULT_ADJUSTMENTS);
  ...
  const updateAdjustment =
    (key: keyof ScenarioAdjustments, min: number, max: number) =>
    ...
    try {
      ...
      if (!actions || actions.length === 0) {
        pushToast({
          ...
          description:
            "The current scenario forecast is already within healthy ranges.",
        });
        return;
      }
      ...
      logger.error("Failed to push copilot actions", {
        error: err,
        tags: ["error"],
      });
      pushToast({
        ...
        description:
          err instanceof Error ? err.message : "Co-Pilot automation failed.",
        variant: "destructive",
      });
    } finally {
  ...
  if (!baseline && loading) {
    return (
      <div
        className={cn(
          "flex h-full min-h-[240px] items-center justify-center rounded-lg border",
          className,
        )}
      >
    ...
    return (
      <div className={cn("space-y-4", className)}>
        <Alert variant="destructive">
          ...
          <AlertDescription>
            We could not load baseline metrics for this workspace. Please
            refresh or check your database connection.
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className={cn("space-y-6", className)}>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-lg border p-4">
              ...
              <Progress
                className="mt-3 h-2"
                value={baseline.scheduling.coverageRate * 100}
              />
              <p className="mt-2 text-xs text-muted-foreground">
                {baseline.scheduling.totalShifts} shifts scheduled ·{" "}
                {baseline.scheduling.openShifts} open
              </p>
            </div>
            ...
            <div className="rounded-lg border p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {baseline.tasks.overdue} overdue · avg age{" "}
                {baseline.tasks.avgAgeDays.toFixed(1)}d
              </p>
            </div>
            ...
            <div className="rounded-lg border p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Margin {percent(baseline.revenue.marginRate, 1)} · Operating
                cost {formatCurrency(baseline.revenue.operatingCost30)}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="flex flex-col">
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Adjust levers to preview what-if impact across staffing,
                  tasks, goals, and revenue.
                </CardDescription>
              </div>
              <Badge
                className={cn(
                  "uppercase tracking-wide",
                  riskStyles[outcome.risk],
                )}
              >
                {outcome.risk} risk
              </Badge>
          ...
          <CardContent className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Positive values add headcount.
                </p>
              </div>
              <div>
                <Label htmlFor="overtime-reduction">
                  Overtime reduction (%)
                </Label>
                <Input
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-muted-foreground">
                  Menu, pricing, partnership shifts.
                </p>
              </div>
              <div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 text-lg font-semibold">
                    {percent(outcome.predicted.coverageRate)}
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-xs uppercase text-muted-foreground">
                    ...
                    <span
                      className={
                        outcome.deltas.goalConfidence >= 0
                          ? "text-emerald-600"
                          : "text-red-600"
                      }
                    >
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Margin {percent(outcome.predicted.marginRate, 1)} (
                    {formatDeltaPercent(outcome.deltas.marginRate)})
                  </p>
                </div>
              </div>
            </div>
            ...
            {lastTriggeredTaskIds.length > 0 && (
              <Alert
                variant="default"
                className="flex flex-wrap items-center gap-3 border border-primary/40 bg-primary/5"
              >
                <div className="flex-1">
                  <AlertTitle>Automation queued</AlertTitle>
                  <AlertDescription>
                    Co-Pilot created {lastTriggeredTaskIds.length} task
                    {lastTriggeredTaskIds.length === 1 ? "" : "s"} from this
                    scenario.
                  </AlertDescription>
                </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Recommended automations generated from this scenario. Triggering
            will create tasks tagged for Co-Pilot follow-up.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {outcome.copilotActions.length === 0 ? (
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
              Scenario looks healthy. Adjust inputs to surface automation
              opportunities.
            </div>
          ) : (
