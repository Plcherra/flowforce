...
const mockFrom = supabase.from as unknown as ReturnType<typeof vi.fn> & { mockReturnValue: any; mockImplementation: any; mockReset: any };
...
describe('messagesRepository', () => {
  ...
  it('listChannels parses and returns channels', async () => {
    const rawChannel = {
      ...
      department_id: null,
      is_private: false,
      channel_members: [{ user_id: 'user-1', role: 'admin', last_read_at: null }],
  ...
  it('insertMessage returns normalized message', async () => {
    ...
    ensureBuilder.maybeSingle.mockResolvedValue({ data: { channel_id: 'channel-1' }, error: null });
    ...
    insertBuilder.single.mockResolvedValue({
      data: {
        ...
        message_type: 'text',
        attachments: null,
        sender_profile: { first_name: 'John', last_name: 'Doe', avatar_url: null },
        reply_to_message: null,
      },
      error: null,
    });

    mockFrom.mockImplementationOnce(() => ensureBuilder).mockImplementationOnce(() => insertBuilder);

    const message = await messagesRepository.insertMessage('channel-1', 'user-1', 'Hello');
  ...
  it('deleteChannel removes messages, members, and channel', async () => {
    ...
    await expect(messagesRepository.deleteChannel('channel-1', 'user-1')).resolves.not.toThrow();
