  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">{snapshot.label}</p>
        ...
        {snapshot.severity && (
          <Badge variant={snapshot.severity === 'normal' ? 'outline' : 'destructive'}>
            {snapshot.severity}
          </Badge>
        )}
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <Progress value={snapshot.severity === 'critical' ? 35 : snapshot.severity === 'warning' ? 65 : 90} />
        <Drawer open={open} onOpenChange={setOpen}>
          ...
          <DrawerContent>
            <DrawerHeader>
              ...
              <DrawerDescription>
                FlowForce will design a script to stabilize {snapshot.label.toLowerCase()}.
              </DrawerDescription>
            </DrawerHeader>
            <div className="space-y-3 px-4 pb-4 text-sm">
              <p>
                Trend: <span className={snapshot.severity ? severityVariants[snapshot.severity] : ''}>{snapshot.trendLabel}</span>
              </p>
              <p>
                The automation generator will craft a task sequence, notifications, and database inserts conforming to the
                Auto-Script DSL. You will be able to review before execution.
              </p>
            </div>
            <DrawerFooter>
              <Button onClick={() => setOpen(false)}>Generate suggestion</Button>
              <DrawerClose asChild>
