    ...
    <Card className="flex flex-col border-border/60 bg-background/80 shadow-sm">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {goal.target_completion_date && (
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              ...
              <span>Due {format(new Date(goal.target_completion_date), 'MMM d, yyyy')}</span>
            </div>
          )}
        </div>
        <CardTitle className="text-xl font-semibold text-foreground">{displayTitle}</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg border border-dashed border-border/70 bg-muted/40 p-3 text-sm">
          ...
          {goal.rewardSummary && (
            <p className="text-xs text-muted-foreground">{goal.rewardSummary}</p>
          )}
        </div>
      </CardContent>

      <CardFooter className="mt-auto flex items-center justify-between gap-2 border-t border-border/60 bg-muted/30 px-4 py-3">
        <Button size="sm" variant="outline" onClick={() => onEdit(goal)} className="gap-2">
          <PencilLine className="h-4 w-4" />
          Edit
        </Button>
        <Button size="sm" variant="ghost" className="gap-2 text-destructive" onClick={() => onDelete(goal)}>
