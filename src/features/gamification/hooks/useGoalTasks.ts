...
const normalizeStatuses = (value: string | string[] | undefined) => {
  ...
  return list.length > 0
    ? list.map((status) => status.toLowerCase())
    : undefined;
};

const mapGoalTaskRow = (
  row: GoalTaskRow & { task?: TaskRow | null },
): GoalTaskLink => ({
  id: row.id,
  goal_id: row.goal_id,
  task_id: row.task_id,
...
export function useGoalTasks(options: UseGoalTasksOptions = {}) {
  ...
  const statusFilter = useMemo(
    () => normalizeStatuses(filters?.status),
    [filters?.status],
  );
  ...
  const goalTasksQuery = useQuery<GoalTaskLink[]>({
    ...
    queryFn: async () => {
      ...
      const mapped = (data ?? []).map((row) =>
        mapGoalTaskRow(row as GoalTaskRow & { task?: TaskRow | null }),
      );
