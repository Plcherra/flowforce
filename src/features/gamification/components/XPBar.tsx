...
export function XPBar({ currentXP, nextMilestone, previousMilestone, loading, className }: XPBarProps) {
  const { progress, xpToGo, baseline } = useMemo(() => {
    ...
    const clampedCurrent = Math.min(Math.max(currentXP, min), nextMilestone.xpRequired);
      ...
      <CardContent className="space-y-4">
        <div>
          <div className="text-3xl font-semibold text-foreground">
            {currentXP.toLocaleString()} <span className="text-base text-muted-foreground">XP</span>
          </div>
        ...
        <div className="relative h-4 rounded-full bg-muted/70">
          <motion.div
            ...
            className={cn('absolute left-0 top-0 h-full rounded-full', gradientClass)}
          />
          <motion.div
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p>{nextMilestone.description ?? 'Complete goals, tasks, and training to level up.'}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="font-medium text-foreground">{nextMilestone.xpRequired.toLocaleString()} XP goal</p>
            {baseline > 0 ? <p>Baseline {baseline.toLocaleString()} XP</p> : null}
          </div>
        </div>
      </CardContent>
    </Card>
