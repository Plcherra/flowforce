...
export function UserPermissionsTab({ user }: UserPermissionsTabProps) {
  ...
  const { data: effectivePermissions } = useUserEffectivePermissions(
    user.id,
    user.role_id,
  );
  ...
  useEffect(() => {
    const overrideMap = PERMISSION_KEYS.reduce(
      (acc, key) => {
  ...
  const handleResetToDefaults = () => {
    const resetOverrides = PERMISSION_KEYS.reduce(
      (acc, key) => {
        acc[key] = "inherit";
        return acc;
      },
  ...
  const getPermissionIcon = (
    ...
    return (
      <Circle
        className={`h-4 w-4 ${effective ? "text-green-600" : "text-muted-foreground"}`}
      />
    );
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Manage granular overrides by category. Use pagination and the
            accordion to focus on the policies you need.
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          <Accordion
          ...
          >
            {paginatedGroups.map(({ category, permissions }) => {
              const overrideCount = permissions.reduce((acc, definition) => {
                const overrideValue =
                  permissionOverrides[definition.key] ?? "inherit";
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {permissions.map((definition) => {
                              const overrideValue =
                                permissionOverrides[definition.key] ??
                                "inherit";
                              ...
                              const effective =
                                effectivePermission?.effective ?? false;
          ...
          {totalPages > 1 && (
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <span className="text-sm text-muted-foreground">
                Viewing categories {startCategoryIndex}–{endCategoryIndex} of{" "}
                {totalCategories}
              </span>
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {savePermissions.isPending || updateUserRole.isPending
                ? "Saving..."
                : "Save Changes"}
            </Button>
          </div>
