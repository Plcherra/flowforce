...
const sortByStart = (list: CalendarEvent[]) =>
  [...list].sort(
    (a, b) =>
      (safeDate(a.start)?.getTime() ?? 0) - (safeDate(b.start)?.getTime() ?? 0),
  );
...
export function CalendarView({
  ...
  const currentDayEvents = useMemo(
    () => sortByStart(eventsByDay.get(dayKey(date)) ?? []),
    [date, eventsByDay],
  );
  ...
  const renderDayColumn = (day: Date, dense?: boolean) => {
    ...
    return (
      <div
        key={key}
        className={cn("rounded-md border p-3", dense ? "" : "min-h-[110px]")}
      >
        <div className="mb-2 flex items-center justify-between text-xs font-semibold">
          <span className={cn(isSameDay(day, new Date()) && "text-primary")}>
            {format(day, "EEE")}
          </span>
