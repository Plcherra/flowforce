...
async function fetchCalendarEventsWithRelations(
  ...
  if (eventIds.length > 0) {
    ...
    const parsedShiftLinks = z
      .array(eventShiftLinkRowSchema)
      .parse(shiftLinkRows ?? []);
...
async function listCompanyEvents(
  companyId: string,
): Promise<CalendarEventRowWithRelations[]> {
...
async function updateEvent(
  id: string,
  updates: TablesUpdate<"calendar_events">,
...
async function replaceEventParticipants(
  ...
  logger.warn(
    "replace_event_participants RPC unavailable, falling back to manual sync",
    { error: rpcResponse.error, tags: ["warning"] },
  );
...
async function replaceEventShiftLinks(
  ...
  logger.warn(
    "replace_event_shift_links RPC unavailable, falling back to manual sync",
    { error: rpcResponse.error, tags: ["warning"] },
  );
