...
function withinRange(dateISO: string | null | undefined, range: PeriodRange, inclusiveEnd = true): boolean {
...
function ensureMetrics(map: Map<string, LeaderboardSyncMetrics>, employeeId: string): LeaderboardSyncMetrics {
...
async function collectTaskMetrics(employeeIds: string[], range: PeriodRange) {
  ...
  (data ?? []).forEach((task) => {
    ...
    const estimateBonus = task.estimated_hours != null ? Math.max(0, Math.round(task.estimated_hours * 10)) : 0;
...
async function collectGoalMetrics(employeeIds: string[], range: PeriodRange) {
  ...
  const metrics = new Map<string, LeaderboardSyncMetrics>();
  (data ?? []).forEach((row) => {
    const goal = row.goal as { status?: string | null; completed_at?: string | null } | null;
...
async function collectTrainingMetrics(employeeIds: string[], range: PeriodRange, companyId: string) {
  ...
  (data ?? []).forEach((row) => {
    ...
    const course = row.course as { title?: string | null; xp_reward?: number | null } | null;
...
function mergeMetrics(metricMaps: Map<string, LeaderboardSyncMetrics>[]): Map<string, LeaderboardSyncMetrics> {
    ...
    periods.map(async (period) => {
      ...
      merged.forEach((metrics, employeeId) => {
        ...
        const row: LeaderboardSyncRow = {
          ...
          department_id: employee.department_id ?? employee.department?.id ?? null,
          role: employee.role,
          period,
