...
export const leaderboardPeriodOptions: { label: string; value: LeaderboardPeriod }[] = [
...
type DepartmentOption = { id: string | null; name: string | null; count: number };
...
export function useLeaderboardFilters({
  ...
  const lastUpdatedLabel = useMemo(() => (lastUpdated ? dayjs(lastUpdated).fromNow() : 'not synced'), [lastUpdated]);

  const periodLabel = useMemo(
    () => leaderboardPeriodOptions.find((option) => option.value === period)?.label ?? 'Custom period',
    [period],
  );
  ...
  const departmentLabel = useMemo(() => {
    ...
    return formatDepartmentName(departments.find((dept) => dept.id === departmentFilter)?.name) ?? 'selected department';
  ...
  const filterStatusMessage = useMemo(
    ...
    [periodLabel, departmentLabel, roleLabel, filteredEntries.length, analytics.participantCount],
  );
