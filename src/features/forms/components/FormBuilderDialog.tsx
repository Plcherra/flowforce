...
import FormFieldLibrary, {
  type FieldTemplate,
} from "@/features/forms/components/editor/FormFieldLibrary";
import FormEditorPanel from "@/features/forms/components/editor/FormEditorPanel";
import FormLivePreview from "@/features/forms/components/editor/FormLivePreview";
...
export default function FormBuilderDialog({
  ...
  const schema = useFormSchema() as FormSchema | null;
      ...
      sections: [
        {
          ...
          fields: rows.map((row) => ({
            ...
            options: Array.isArray(row.options)
              ...
              : [],
            validation:
              (row.validation_rules as FormField["validation"]) ?? undefined,
            ...
            dependent_fields: Array.isArray(row.dependent_fields)
              ...
              : undefined,
            rating_config:
              (row.rating_config as FormField["rating_config"]) ?? undefined,
            scan_config:
              (row.scan_config as FormField["scan_config"]) ?? undefined,
            media_config:
              (row.media_config as FormField["media_config"]) ?? undefined,
            content: undefined,
          })),
        },
      ],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (updateResult.error) {
        toast({
          title: "Save failed",
          description: "Unable to update form details.",
          variant: "destructive",
        });
        return;
      }
      ...
      const payload: SaveField[] = fields.map((field, index) => ({
        ...
        media_config: field.media_config ? (field.media_config as unknown as Record<string, unknown>) : null,
      }));
      ...
      if (error) {
        toast({
          title: "Save failed",
          description: "Unable to save form changes.",
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: "Form saved",
        description: "All changes have been stored.",
      });
      onOpenChange(false);
    } finally {
      setIsSaving(false);
  ...
  const fieldCount =
    schema?.sections.reduce((sum, section) => sum + section.fields.length, 0) ??
    0;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[85vh] max-h-[calc(100vh-3rem)] w-full max-w-[1200px] flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="flex-shrink-0 space-y-1 border-b px-6 py-4">
          ...
          <DialogDescription>
            Compose fields, configure options, and preview the live form
            experience.
          </DialogDescription>
