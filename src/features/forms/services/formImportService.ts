// Note: ingestion/api is still in src/services/ingestion/ as it's shared across features
...
export async function importFormFromFile(file: File, userId: string) {
  ...
  try {
    ...
    try {
      ...
      logger.warn("Unable to extract raw text from uploaded file", {
        error,
        tags: ["warning"],
      });
    }
    ...
    logger.warn(
      "Document ingestion failed – continuing without document linkage",
      { error, tags: ["warning"] },
    );
  }
