...
const HOUR_OPTIONS: HourOption[] = Array.from({ length: 24 }).map(
  (_, index) => ({
...
async function invalidateAvailabilityQueries(
  ...
  await Promise.all([
    ...
    queryClient.invalidateQueries({
      queryKey: ["availability-exceptions", orgId ?? null],
    }),
  ]);
}
...
export function useAvailabilityManagement({
  ...
  const orgPrefsQuery = useQuery<OrgPrefs | null>({
    ...
    queryFn: async () => {
      ...
      return {
        id: data.id,
        availabilityLockMode:
          data.availability_lock_mode as AvailabilityLockMode,
  ...
  const requestsQuery = useQuery({
    ...
    queryFn: async (): Promise<ManagerAvailabilityRequest[]> => {
      const { data: requests, error } = await supabase
        ...
        .select(
          "id, employee_id, week_start, payload, status, manager_id, decision_note, created_at, updated_at",
        )
      ...
      const employeeIds = Array.from(
        new Set(requests.map((req) => req.employee_id)),
      ).filter(Boolean);
      const weekStarts = Array.from(
        new Set(requests.map((req) => req.week_start)),
      ).filter(Boolean);
      ...
      return requests.map((request) => {
        ...
        const baselineRows =
          availabilityMap.get(`${request.employee_id}-${request.week_start}`) ??
          [];
  ...
  const approveRequestMutation = useMutation({
    mutationFn: async (request: ManagerAvailabilityRequest) => {
      ...
      const exceptionInsert = await supabase
        .from("availability_exception")
        .insert({
