...
function calculateAvailabilityHours(grid?: AvailabilityGrid) {
  ...
  return Object.values(grid).reduce(
    (total, hours) => total + (Array.isArray(hours) ? hours.length : 0),
    0,
  );
}
...
export function TimeOffRequestPanel({
  employeeId,
  availabilityGrid,
}: TimeOffRequestPanelProps) {
  const { toast } = useToast();
  const { timeOff, shifts, loading, isFallbackData, mutations } =
    useScheduling();
  ...
  const availabilityHours = useMemo(
    () => calculateAvailabilityHours(availabilityGrid),
    [availabilityGrid],
  );
  ...
  const approvedDays = useMemo(
    () =>
      personalRequests
        ...
        .reduce(
          (total, request) =>
            total + dayCount(request.start_date, request.end_date),
          0,
        ),
    [personalRequests],
  );
  ...
  const allowanceRemaining = Math.max(
    BASE_TIME_OFF_ALLOWANCE - approvedDays,
    0,
  );
    ...
    return shifts.filter((shift) => {
      ...
      const assigned =
        ...
        shift.assignments?.some(
          (assignment) => assignment.user_id === employeeId,
        );
  ...
  const handleSubmitRequest = async () => {
    if (!employeeId) {
      toast({
        ...
        description:
          "We could not resolve your profile. Refresh and try again.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!requestStart || !requestEnd) {
      toast({
        ...
        description:
          "Select both a start and end date for your time off request.",
        variant: "destructive",
      });
      return;
    }
    ...
    return (
      <Card className="border bg-background shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            Connect to Supabase to sync real time-off requests and balances.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="rounded-md border border-dashed border-muted-foreground/40 p-4 text-sm text-muted-foreground">
            Scheduling data is in preview mode, so we can&apos;t show live
            balance or request history yet.
          </div>
        </CardContent>
      </Card>
    );
    ...
    <Card className="border bg-background shadow-sm">
      <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <CardDescription>
            Track requests, balances, and how time off overlaps with your
            scheduled shifts.
          </CardDescription>
        </div>
        <Dialog open={requestDialogOpen} onOpenChange={setRequestDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Choose the date range and type of time off. Your supervisor will
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Select
                  value={requestType}
                  onValueChange={(value) =>
                    setRequestType(value as TimeOffType)
                  }
                >
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            {personalRequests.length > 0 && (
              <span className="text-xs text-muted-foreground">
                Updated&nbsp;
                {format(
                  new Date(personalRequests[0].created_at ?? new Date()),
                  "MMM d, yyyy",
                )}
              </span>
            )}
          ...
          {personalRequests.length === 0 ? (
            <div className="mt-3 rounded-lg border border-dashed border-muted-foreground/40 p-6 text-sm text-muted-foreground">
              No time off requests yet. Submit your first request to see it
              tracked here alongside coverage impact.
            </div>
          ) : (
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge
                              className={cn(
                                ...
                                TYPE_STYLES[
                                  (request.type as TimeOffType) ?? "other"
                                ] ?? TYPE_STYLES.other,
                              )}
                            >
                              {TIME_OFF_TYPE_LABEL[
                        ...
                        <Badge
                          className={cn(
                            "border text-xs capitalize",
                            STATUS_STYLES[
                              (request.status as TimeOffStatus) ?? "pending"
