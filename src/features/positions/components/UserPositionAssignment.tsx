...
export function UserPositionAssignment({
  children,
}: UserPositionAssignmentProps) {
  const {
    positions,
    assignments,
  ...
  const isUserAssignedToPosition = (userId: string, positionId: string) => {
    return assignments.some(
      (a) =>
        a.user_id === userId && a.position_id === positionId && a.is_active,
    );
  };
                          ...
                          userPositions.map((position) => (
                            ...
                            >
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleRemovePosition(user.id, position.id)
                                }
                              >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {positions
                          .filter(
                            (position) =>
                              !isUserAssignedToPosition(user.id, position.id),
                          )
                          .map((position) => (
                            <Button
                              key={position.id}
                              ...
                              variant="outline"
                              onClick={() =>
                                handleAssignPosition(user.id, position.id)
                              }
