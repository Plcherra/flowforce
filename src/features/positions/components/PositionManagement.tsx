...
type ShiftTemplateRecord = ReturnType<typeof useShiftTemplates>['templates'][number];
...
const generateCopilotSuggestions = (
  ...
  ROLE_ORDER.forEach((role) => {
    const rolePositions = positions.filter((position) => position.role === role);
...
export default function PositionManagement() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'positions' | 'permissions'>(
    'overview',
  );
  ...
  const unlinkedShiftTemplateCount = shiftTemplates.length - linkedShiftTemplateCount;
  ...
  const roleAccessSummaries = useMemo<RoleAccessSummary[]>(() => {
    return rolesArray.map((role) => {
      ...
      const positionCount = matchingRoleKey
        ? positions.filter((position) => position.role === matchingRoleKey).length
        : 0;
    ...
    <RoleGuard
      ...
      fallback={
        <div className="p-8 text-center">
          ...
          <p className="text-gray-600">You do not have permission to manage positions.</p>
        </div>
      }
    >
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Keep positions, scheduling templates, and access control moving together.
            </p>
          </div>
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              <div className="grid gap-4 xl:grid-cols-3">
                <Card className="xl:col-span-2">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Connect position templates to recurring shifts so the schedule stays in sync.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    {unlinkedShiftTemplateCount > 0 && (
                      <Alert>
                        ...
                        <AlertDescription>
                          Co-Pilot spotted shift templates without a position. Link them so
                          assignments inherit the right permissions.
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    <div className="space-y-3">
                      {shiftTemplates.length === 0 ? (
                        <div className="text-sm text-muted-foreground">
                          No shift templates yet. Create a template from Enhanced Scheduling to
                          jump-start staffing suggestions.
                        </div>
                      ) : (
                        shiftTemplates.slice(0, 5).map((template) => {
                          const linkedRole = template.job_position?.role as RoleKey | undefined;
                          const positionName = template.job_position?.name ?? 'Unassigned template';
                          return (
                            <div
                              key={template.id}
                            ...
                            >
                              ...
                              {linkedRole ? (
                                <Badge className={getRoleBadgeClass(linkedRole)}>
                                  {ROLE_LABELS[linkedRole]}
                                </Badge>
                              ) : (
                  ...
                  <CardFooter className="flex justify-end">
                    <Button variant="ghost" size="sm" asChild>
                      <Link to="/enhanced-scheduling">Open Enhanced Scheduling</Link>
                    </Button>
                  </CardFooter>
                </Card>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      AI recommendations based on positions, scheduling, and permissions.
                    </CardDescription>
                  </CardHeader>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Understand how role templates translate into application access.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {roleAccessSummaries.length === 0 ? (
                    <div className="text-sm text-muted-foreground">
                      Configure company roles to automatically sync section entitlements for each
                      position template.
                    </div>
                  ) : (
                    roleAccessSummaries.slice(0, 4).map((summary) => (
                      <div
                        ...
                        style={{
                          borderColor: summary.color ? `${summary.color}33` : undefined,
                          backgroundColor: summary.color ? `${summary.color}0a` : undefined,
                        }}
                      >
                        ...
                        <div className="flex flex-wrap gap-1">
                          {summary.accessibleSections.length === 0 ? (
                            ...
                            summary.accessibleSections.slice(0, 5).map((section) => (
                              <Badge variant="secondary" key={section} className="text-xs">
                                {section}
                              </Badge>
                            ))
                          )}
                ...
                <CardFooter className="flex justify-end">
                  <Button variant="ghost" size="sm" asChild>
                    <Link to="/sections-permissions">Manage Sections & Permissions</Link>
                  </Button>
                </CardFooter>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="positions" className="space-y-6">
              {positions.length === 0 && (
                <Alert>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    No position templates found. Create one to start assigning team members and
                    connecting scheduling templates.
                  </AlertDescription>
                </Alert>
              )}

              <div className="grid gap-4 md:grid-cols-2">
                {positionsByRole.map(({ role, label, positions: rolePositions }) => (
                  <Card key={role} className="h-full">
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                        <Badge className={getRoleBadgeClass(role)}>{label}</Badge>
                    ...
                    <CardContent className="space-y-3">
                      {rolePositions.length === 0 ? (
                        <div className="text-sm text-muted-foreground">
                          No {label.toLowerCase()} position templates yet. Create one to unlock role
                          defaults and scheduling coverage.
                        </div>
                      ) : (
                        rolePositions.map((position) => (
                          <div
                            ...
                            style={{
                              borderColor: position.color ? `${position.color}33` : undefined,
                              backgroundColor: position.color ? `${position.color}0a` : undefined,
                            }}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {(shiftTemplatesByPositionId.get(position.id) ?? 0)} linked shift
                                  template
                                  {(shiftTemplatesByPositionId.get(position.id) ?? 0) === 1
                                    ? ''
                                    : 's'}{' '}
                                  • {(assignmentsByPosition.get(position.id) ?? 0)} active
                                  assignment
                                  {(assignmentsByPosition.get(position.id) ?? 0) === 1 ? '' : 's'}
                                </div>
                              </div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              ...
                              Object.values(position.permissions).some(Boolean) ? (
                              ...
                              {position.permissions &&
                                Object.values(position.permissions).filter(Boolean).length > 4 && (
                                  <Badge variant="outline" className="text-xs">
                                    +
                                    {Object.values(position.permissions).filter(Boolean).length -
                                      4}{' '}
                                    more
                                  </Badge>
                                )}
            ...
            <TabsContent value="permissions" className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Preview default permissions and section coverage for each role template.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {roleAccessSummaries.length === 0 ? (
                    <div className="text-sm text-muted-foreground">
                      No company roles loaded yet. Configure role templates under Sections &
                      Permissions to unlock default access per position.
                    </div>
                  ) : (
                    roleAccessSummaries.map((summary) => {
                      const activePermissions = Object.entries(summary.permissions)
                        .filter(([, value]) => value)
                        .map(([key]) => key);
                        ...
                        >
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              ...
                              {summary.accessibleSections.length === 1 ? '' : 's'} enabled
                            </span>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {activePermissions.length === 0 ? (
                                ...
                                activePermissions.slice(0, 6).map((permissionKey) => (
                                  <Badge variant="outline" key={permissionKey} className="text-xs">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {summary.accessibleSections.length === 0 ? (
                                ...
                                summary.accessibleSections.slice(0, 6).map((section) => (
                                  <Badge variant="secondary" key={section} className="text-xs">
                                    {section}
                                  </Badge>
                                ))
                ...
                <CardFooter className="flex justify-end">
                  <Button variant="ghost" size="sm" asChild>
                    <Link to="/sections-permissions">Open Sections & Permissions</Link>
                  </Button>
                </CardFooter>
              </Card>
            </TabsContent>
