...
const parseWithSchema = <Schema extends z.ZodTypeAny>(
  ...
  if (!result.success) {
    logger.error(`[learning] Invalid payload for ${context}`, {
...
const mapCourse = (row: CourseRow): LearningCourse => ({
  ...
  deliveryMode:
    (row.delivery_mode as LearningCourse["deliveryMode"]) ?? "self_paced",
...
const mapModule = (row: ModuleRow): LearningModule => {
  ...
  if (row.content) {
    try {
      ...
      logger.warn("Unable to parse module content metadata", {
        error,
        tags: ["warning"],
      });
    }
...
const mapMetrics = (row: MetricsRow): LearningCourseMetrics => ({
  ...
  totalHoursCompleted:
    row.total_hours_completed != null
      ? toNumber(row.total_hours_completed)
      : null,
  totalXpAwarded:
    row.total_xp_awarded != null ? toNumber(row.total_xp_awarded) : null,
});
...
export async function fetchLearningCatalog(
  companyId: string,
): Promise<LearningCatalogRecord[]> {
  if (!companyId) {
    throw new Error(
      "Company context is required to fetch the learning catalog.",
    );
  }
...
export async function createLearningCourse(
  ...
  const { data: courseRow, error: insertCourseError } =
    await fromTable<CourseRow>(TABLE_COURSES)
      .insert({
        slug,
        title: payload.title,
  ...
  const courseRecord = parseWithSchema(
    courseRowSchema,
    courseRow,
    TABLE_COURSES,
  );
  ...
  if (modulesPayload.length > 0) {
    const { error: moduleError } =
      await fromTable(TABLE_MODULES).insert(modulesPayload);
    if (moduleError) {
...
export async function updateEnrollmentProgress(
  ...
  const snapshotPayload: TablesInsert<"learning_progress"> = {
    ...
    progress_percent:
      updates.progressPercent ?? toNumber(data.progress_percent),
    time_spent_minutes:
      options.timeSpentMinutes ??
      Math.max(
        0,
        Math.round(
          toNumber(updates.hoursCompleted ?? data.hours_completed) * 60,
        ),
      ),
  ...
  try {
    ...
    logger.warn("[learning] unable to persist progress snapshot", {
      error: snapshotError,
      tags: ["warning"],
    });
  }
...
export async function fetchProgressSnapshots(
  ...
  if (error) throw error;
  const rows = parseWithSchema(
    progressSnapshotRowSchema.array(),
    data ?? [],
    TABLE_PROGRESS_SNAPSHOTS,
  );
...
export async function fetchProgressHistoryPage(params: {
  ...
  if (!parsed.success) {
    logger.error("[learning] Invalid progress history payload", {
...
export async function fetchCourseMetrics(
  companyId: string,
): Promise<LearningCourseMetrics[]> {
  if (!companyId) {
    throw new Error(
      "Company context is required to fetch learning course metrics.",
    );
  }
...
export function buildPersonalSnapshot(
  ...
  const totalHours = enrollments.reduce(
    (sum, item) => sum + item.hoursCompleted,
    0,
  );
  const avgProgress =
...
export async function recordCourseCompletionMetadata(
  ...
  options: {
    companyId: string;
    employeeId: string;
    awardingProfileId?: string | null;
  },
) {
  if (!options.companyId) {
    throw new Error(
      "Company context is required to record course completion metadata.",
    );
  }

  const { data: completionRow, error: completionLookupError } =
    await learningClient
  ...
  if (!completionRow) {
    const { error: completionInsertError } = await learningClient
      .from("learning_completions")
      .insert({
        employee_id: options.employeeId,
...
function buildCourseRecommendations(
  ...
  const metricsById = new Map(
    metrics.map((metric) => [metric.courseId, metric]),
  );
...
export async function getCourseRecommendations(options: {
  ...
  const recommendations = buildCourseRecommendations(
    catalog,
    enrollments,
    metrics,
    {
