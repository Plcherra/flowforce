...
vi.mock("@/features/gamification/leaderboard/useLeaderboardData", () => ({
...
describe("Recognition page smoke scenarios", () => {
  it("disables manual action while employees are loading and avoids empty state during initial skeleton", () => {
    ...
    expect(
      screen.getByRole("button", { name: /give recognition/i }),
    ).toBeDisabled();
  ...
  it("shows inline validation errors when submitting empty manual form", () => {
    ...
    expect(
      screen.getByText(/Select a teammate to recognize/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Add a brief recognition message/),
    ).toBeInTheDocument();
  ...
  it("surface roster error alert when employees API fails", () => {
    ...
    expect(
      screen.getByText(/Employee directory unavailable/i),
    ).toBeInTheDocument();
