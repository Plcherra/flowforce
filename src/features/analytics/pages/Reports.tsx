...
export default function ReportsPage() {
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return documents.filter((doc) => {
      const text =
        `${doc.title ?? ""} ${doc.source ?? ""} ${doc.file?.filename ?? ""}`.toLowerCase();
        ...
        <Card>
          <CardContent className="py-16 text-center space-y-3">
            ...
            <p className="text-muted-foreground">
              You do not have permission to view reporting data. Contact an
    ...
    <div className="container mx-auto px-6 py-8 space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Upload daily PDFs, review extracted insights, and launch follow-up
            actions in minutes.
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-14 text-muted-foreground">
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-sm">
                  Upload your first daily report to kick off automatic event
                  extraction.
                </p>
              </div>
            </div>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {doc.source ?? "Uploaded"} ·{" "}
                            {doc.file?.mime_type ?? "Unknown format"}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <Badge
                          className={`flex items-center gap-1 ${statusClass(status.tone)}`}
                        >
                          <Icon
                            className={`h-3.5 w-3.5 ${status.tone === "warning" ? "animate-spin" : ""}`}
                          />
                          {status.label}
                        </Badge>
                      </TableCell>
        ...
        <SheetContent side="right" className="w-full sm:max-w-xl">
          {selectedDocument ? (
            <div className="flex h-full flex-col">
              ...
              {selectedDocument.processing_state === "error" &&
              ...
              <div className="mt-4 space-y-4">
                <Card>
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription>
                      Uploaded{" "}
                      {selectedDocument.file?.uploaded_at
                        ? format(
                            new Date(selectedDocument.file.uploaded_at),
                            "PPP h:mm a",
                          )
                        : "unknown"}
                    </CardDescription>
                  ...
                  <CardContent className="space-y-3 text-sm text-muted-foreground">
                    ...
                    <p>
                      ...
                      {selectedDocument.doc_date
                        ? format(new Date(selectedDocument.doc_date), "PPP")
                        : "Not provided"}
                    </p>
                ...
                <Card>
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription>
                      Top of document content (first 600 characters)
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                  ...
                  <CardContent>
                    {selectedDocument.events.length === 0 ? (
                      <p className="text-sm text-muted-foreground">
                        No structured events recorded. Once processing
                        completes, rules will surface key incidents here.
                      </p>
                    ) : (
                      <div className="space-y-3">
