...
const toneClasses: Record<InsightTone, string> = {
  ...
  green:
    "border border-emerald-400/30 bg-emerald-500/10 text-emerald-700 dark:border-emerald-300/40",
    ...
    return (
      <Card className="border-dashed bg-muted/20">
        ...
        <CardContent className="text-sm text-muted-foreground">
          Connect performance reviews and goals to unlock AI summaries and XP
          forecasts.
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {metrics.map((metric) => (
        <Card
          key={metric.id}
          className={`${toneClasses[metric.tone ?? "slate"]} shadow-sm`}
        >
