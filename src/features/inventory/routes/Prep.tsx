...
export default function PrepParPage() {
  ...
  const completedCount = Object.values(prepStatuses).filter(status => status === 'completed').length;
  const inProgressCount = Object.values(prepStatuses).filter(status => status === 'in_progress').length;
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-muted-foreground">
                {isWeekend() ? 'Weekend' : 'Weekday'} prep planning • {new Date().toLocaleDateString()}
              </p>
            </div>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="text-2xl font-bold">
                  {mockPrepItems.length > 0 ? Math.round((completedCount / mockPrepItems.length) * 100) : 0}%
                </div>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
                            ...
                            <div>
                              <span className="text-muted-foreground">PAR Min/Max:</span>
                              <div className="font-medium">{item.parMin}/{item.parMax} {item.unit}</div>
                            </div>
                            <div>
                              <span className="text-muted-foreground">Need to Prep:</span>
                              <div className="font-medium text-primary">{needed} {item.unit}</div>
                            </div>
                            <div>
                              <span className="text-muted-foreground">Estimated Batches:</span>
                              <div className="font-medium text-primary">≈ {batches}</div>
                            </div>
                          </div>
                          
                          <p className="text-xs text-muted-foreground mt-2">{item.batchYield}</p>
                        </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {completedCount} of {mockPrepItems.length} prep items completed
                  </p>
                </div>
                <div className="flex gap-2">
