...
import { AlertTriangle, Package, MapPin, Calendar, DollarSign } from 'lucide-react';
...
export default function WasteTrackingPage() {
  ...
  const totalWasteValue = wasteRecords.reduce((sum, record) => sum + (record.cost_impact || 0), 0);
      ...
      <IfCan permission="inventory.waste.view">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Total Quantity Wasted</CardTitle>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">${totalWasteValue.toFixed(2)}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <CardContent>
              {wasteRecords.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No waste records found</h3>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <Package className="h-4 w-4 text-muted-foreground" />
                            <span className="font-medium">{record.item?.name || 'Unknown Item'}</span>
                            <Badge className={wasteTypeColors[record.waste_type]}>
                          ...
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{record.quantity} {record.item?.unit?.name || 'units'}</span>
                            </div>
