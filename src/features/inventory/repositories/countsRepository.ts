...
import { z } from 'zod';
import type { InventoryCount, InventoryCountLine, InventoryItem, InventoryItemUnit } from '@/features/inventory/hooks/types';
...
type ScopedCountItem = {
  item: Pick<
    InventoryItem,
    'id' | 'name' | 'category' | 'sku' | 'default_location_id' | 'unit_id' | 'unit' | 'unit_quantity'
  >;
  expectedQuantity: number;
  units: InventoryItemUnit[];
};

export async function listInventoryCounts(options: CountRepositoryOptions = {}): Promise<InventoryCount[]> {
...
export async function getInventoryCount(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const { data, error } = await client
    .from('inv_counts')
    .select(
...
export async function createInventoryCount(payload: CreateInventoryCountInput, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  ...
  if (payload.locations.length > 0) {
    const locationPayload = payload.locations.map((locationId) => ({ count_id: countId, location_id: locationId }));
    const { error: locationError } = await client.from('inv_count_locations').upsert(locationPayload, { onConflict: 'count_id,location_id' });
...
export async function updateInventoryCount(
  ...
  options: CountRepositoryOptions & { eventType?: string; payload?: Record<string, unknown> } = {},
) {
  ...
  const { error } = await client.from('inv_counts').update(updates).eq('id', countId);
  if (error) throw error;
...
export async function deleteInventoryCount(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;

  const { error: linesError } = await client.from('inv_count_lines').delete().eq('count_id', countId);
...
export async function submitInventoryCount(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const timestamp = new Date().toISOString();

  const { error } = await client
    .from('inv_counts')
    .update({ status: 'awaiting_review', review_status: 'under_review', submitted_at: timestamp })
    .eq('id', countId);
  if (error) throw error;

  await logCountEvent(client, countId, 'submitted', { submitted_at: timestamp });
}

export async function approveInventoryCount(countId: string, input: ReviewActionInput = {}, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  ...
  await logCountEvent(client, countId, 'approved', { reviewed_at: timestamp, notes: input.notes });
}

export async function rejectInventoryCount(countId: string, input: ReviewActionInput = {}, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const reviewerId = await getCurrentUserId(client);
  const timestamp = new Date().toISOString();
  ...
  await logCountEvent(client, countId, 'rejected', { reviewed_at: timestamp, notes: input.notes });
}

export async function completeInventoryCount(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const timestamp = new Date().toISOString();
  ...
  await logCountEvent(client, countId, 'submitted', { completed_at: timestamp });
}

export async function listInventoryCountEvents(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const { data, error } = await client
...
export async function listInventoryCountScans(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const { data, error } = await client
...
export async function recordInventoryCountScan(
  countId: string,
  scannedCode: string,
  options: CountRepositoryOptions & { itemId?: string; scanType?: 'barcode' | 'qr_code'; metadata?: Record<string, unknown> } = {},
) {
  const client = options.supabaseClient ?? supabase;
  const userId = await getCurrentUserId(client);
  const { error } = await client
...
export async function listInventoryCountLines(countId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const { data, error } = await client
  ...
  if (error) throw error;
  return inventoryCountLineArraySchema.parse(data ?? []) as InventoryCountLine[];
}

export async function addInventoryItemToCount(countId: string, itemId: string, expectedQuantity = 0, options: CountRepositoryOptions = {}) {
  return addInventoryItemsToCount(countId, [{ id: itemId, expectedQuantity }], options);
}
...
export async function removeInventoryItemFromCount(lineId: string, options: CountRepositoryOptions = {}) {
  const client = options.supabaseClient ?? supabase;
  const { data, error } = await client
    .from('inv_count_lines')
    .delete()
...
async function resolveActiveCompanyId(client: SupabaseClient, explicit?: string): Promise<string> {
...
async function logCountEvent(client: SupabaseClient, countId: string, eventType: string, payload: Record<string, unknown> = {}) {
...
async function resolveCountScopeItems(client: SupabaseClient, countData: CreateInventoryCountInput, companyId: string): Promise<ScopedCountItem[]> {
  if (countData.items && countData.items.length > 0) {
    return buildScopedItemsForIds(client, countData.items.map((item) => ({ id: item.id, expectedQuantity: item.expectedQuantity ?? 0 })), companyId);
  }

  let itemQuery = client
    .from('inv_items')
    .select(`id, name, category, sku, default_location_id, unit_id, unit:inv_units(*)`)
...
async function buildScopedItemsForIds(
  ...
  const expectedMap = new Map(items.map((entry) => [entry.id, entry.expectedQuantity ?? 0]));
  const scoped = ((data ?? []) as InventoryItem[]).map<ScopedCountItem>((item) => ({
    item,
...
async function attachUnitsToItems(client: SupabaseClient, scopedItems: ScopedCountItem[]) {
  ...
  const unitsByItem = scopedItems.reduce<Record<string, InventoryItemUnit[]>>((acc, entry) => {
    acc[entry.item.id] = [];
    return acc;
  }, {});
  ...
  return scopedItems.map((entry) => ({
    ...entry,
    units: (unitsByItem[entry.item.id] || []).sort((a, b) => (a.unit_level ?? 0) - (b.unit_level ?? 0)),
  }));
}

async function bootstrapCountLines(client: SupabaseClient, countId: string, scopedItems: ScopedCountItem[]) {
  if (!scopedItems.length) return;
  ...
  const existingKeys = new Set<string>((existingLines ?? []).map((line) => `${line.item_id}-${line.unit_id ?? 'base'}`));
