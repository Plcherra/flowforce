...
export default function InventoryItemForm({
  ...
  useEffect(() => {
    if (editItem) {
      ...
      const location = locations?.find(
        (loc) => loc.id === editItem.default_location_id,
      );
      setLocationInput(location?.name || "");

      const supplier = suppliers?.find(
        (sp) => sp.id === editItem.preferred_supplier_id,
      );
  ...
  const locationExists = locations?.some(
    (loc) => loc.name.toLowerCase() === locationInput.toLowerCase(),
  );

  const supplierExists = suppliers?.some(
    (supplier) => supplier.name.toLowerCase() === supplierInput.toLowerCase(),
  );
  ...
  const handleDeleteLocation = async (
    ...
    if (
      confirm(`Are you sure you want to delete the location "${locationName}"?`)
    ) {
      try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!profile?.company_id) {
      toast({
        ...
        description:
          "Please sign in again to continue creating inventory items.",
        variant: "destructive",
      });
      return;
    }
    ...
    const parsedYieldQuantity = formData.recipe_yield_quantity
      ...
      : undefined;
    const recipeYieldQuantity =
      parsedYieldQuantity && parsedYieldQuantity > 0
        ? parsedYieldQuantity
        : undefined;
      ...
      if (savedItem?.id && unitLevels.length > 0) {
        for (const unitLevel of unitLevels) {
          ...
          const payload = {
            ...
            is_countable: true,
            cost_per_unit: unitLevel.cost_per_unit
              ? parseFloat(unitLevel.cost_per_unit)
              : null,
          };
          ...
          try {
            ...
            logger.warn("Failed to upsert unit level", {
              error: unitError,
              tags: ["warning"],
            });
          }
        ...
        for (const removedId of removedUnitIds) {
          try {
            ...
            logger.warn("Failed to delete unit level", {
              error: deleteError,
              tags: ["warning"],
            });
          }
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Click on field labels to enable/disable them
          </p>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <ClickableLabel
                ...
                onClick={() =>
                  setFieldConfig({ ...fieldConfig, sku: !fieldConfig.sku })
                }
              >
                SKU
              </ClickableLabel>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (matchingSupplier) {
                        setFormData({
                          ...formData,
                          preferred_supplier_id: matchingSupplier.id,
                        });
                      } else {
          ...
          <div>
            <ClickableLabel
              ...
              onClick={() =>
                setFieldConfig({
                  ...fieldConfig,
                  unitTypeSize: !fieldConfig.unitTypeSize,
                })
              }
            >
              Units Configuration
                  ...
                  >
                      ...
                      {unitLevels.length > 1 && index > 0 && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const newUnits = unitLevels.filter(
                              (_, i) => i !== index,
                            );
                            const reindexedUnits = newUnits.map((u, idx) => ({
                              ...u,
                              unit_level: idx + 1,
                            }));
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          step="0.01"
                          value={
                            index === 0 ? "1" : unit.conversion_factor || ""
                          }
                          onChange={(e) => {
                            ...
                            const updatedUnits =
                              calculateDerivedPrices(newUnits);
                    ...
                    {index > 0 && (
                      <p className="text-xs text-muted-foreground">
                        Set the number of base units contained in this unit
                        (e.g., 1 Case = 12 Each).
                      </p>
                    )}
                  </div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (matchingLocation) {
                          setFormData({
                            ...formData,
                            default_location_id: matchingLocation.id,
                          });
                        } else {
                          setFormData({ ...formData, default_location_id: "" });
                        }
            ...
            <div>
              <ClickableLabel
                ...
                onClick={() =>
                  setFieldConfig({
                    ...fieldConfig,
                    shelfLife: !fieldConfig.shelfLife,
                  })
                }
              >
                Expiration Period
            ...
            <div className="mt-2 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-center">
              <Input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    recipe_yield_quantity: e.target.value,
                  })
                }
              />
              <Select
          ...
          <div>
            <ClickableLabel
              ...
              onClick={() =>
                setFieldConfig({
                  ...fieldConfig,
                  stockLevels: !fieldConfig.stockLevels,
                })
              }
            >
              Stock Levels
