...
const listComments = async ({ companyId, updateIds }: ListCommentsParams) => {
  ...
  const { data, error } = await supabase
    ...
    .select('*, author:profiles!company_update_comments_author_id_fkey (id, first_name, last_name, avatar_url)')
    .eq('company_id', companyId)
    .in('update_id', updateIds)
...
const updateStatus = async ({ companyId, updateId, status }: UpdateStatusParams) => {
...
const upsertReaction = async ({ companyId, updateId, userId, reactionType }: ReactionMutationParams) => {
  const { error } = await supabase
    .from('company_update_reactions')
    .upsert(
      {
...
const deleteReaction = async ({ companyId, updateId, userId, reactionType }: DeleteReactionParams) => {
  const { error } = await supabase
    .from('company_update_reactions')
    .delete()
    .eq('company_id', companyId)
...
const createComment = async ({ companyId, updateId, userId, content }: CreateCommentParams) => {
