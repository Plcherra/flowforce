...
export function RecognitionFeed({ recognitions, loading }: RecognitionFeedProps) {
    ...
    return (
      <Card className="border-dashed">
        ...
        <CardContent className="text-sm text-muted-foreground">
          Once teammates complete goals, tasks, or training, their shout-outs will be listed here.
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        <CardTitle className="text-base">Recognition feed</CardTitle>
        <CardDescription>Latest shout-outs across the organisation</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {recognitions.slice(0, 12).map((recognition) => {
          const recipient = recognition.recipient;
          const fullName = recipient
            ? `${recipient.first_name ?? ''} ${recipient.last_name ?? ''}`.trim() || 'Unknown employee'
            : 'Unknown employee';
          const message = recognition.reward_details?.message ?? 'Recognition awarded';
          const source = recognition.reward_details?.source ?? 'manual';

          return (
            <div key={recognition.id} className="flex items-center gap-3 rounded-xl border bg-muted/20 p-3">
              <Avatar className="h-10 w-10">
                <AvatarImage src={recipient?.avatar_url ?? undefined} alt={fullName} />
