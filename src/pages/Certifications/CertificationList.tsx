...
export function CertificationList({ certifications, loading, onSelect }: CertificationListProps) {
          ...
          >
            <CardHeader className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                {cert.description && <CardDescription className="mt-2">{cert.description}</CardDescription>}
              </div>
            ...
            <CardContent className="space-y-4">
              <div>
                <p className="text-sm font-medium">
                  {cert.progressPercent}% complete · {cert.requirementDetails.length} requirements
                </p>
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                {cert.requirementDetails.map((detail) => (
                  <p key={detail.key}>
                    {detail.label}: {Math.min(Math.round(detail.current), Math.round(detail.target))}/{Math.round(detail.target)}
                  </p>
                ))}
              </div>
            </CardContent>
