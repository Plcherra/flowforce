...
import { QUICK_TEMPLATES, getTemplatesByCategory } from '@/data/sectionTemplates';
...
export default function AddSection() {
  ...
  const categories = [
    { key: 'communication', label: 'Communication', description: 'Connect and inform your team' },
    { key: 'operations', label: 'Operations', description: 'Manage daily operations and workflows' },
    { key: 'hr', label: 'HR & People', description: 'Human resources and employee management' }
  ];
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Add New Section</h1>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Choose a template or create from scratch</p>
                </div>
              </div>
            ...
            {categories.map((category) => (
              <TabsContent key={category.key} value={category.key} className="space-y-6">
                <div className="text-center">
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{category.label}</h2>
                  ...
                  {getTemplatesByCategory(category.key).map((template) => (
                    ...
                    >
                      ...
                      <CardContent>
                        ...
                        {template.config.pages && template.config.pages.length > 0 && (
                          <div className="mt-4">
                            <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">
                              Includes:
                            </p>
                            <div className="flex flex-wrap gap-1">
                              {template.config.pages.slice(0, 3).map((page, index) => (
