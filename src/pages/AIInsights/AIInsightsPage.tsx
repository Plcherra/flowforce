...
import type { PerformanceDataset, PerformanceGoalReview, PerformanceRadarMetric } from '@/services/performance/performanceTypes';
import { formatDistanceToNow } from 'date-fns';
import { InsightsHeader } from './InsightsHeader';
import { InsightSummaryCards, type SummaryMetric } from './InsightSummaryCards';
import { RecommendationsPanel, type Recommendation } from './RecommendationsPanel';
...
type Trajectory = {
  ...
  previousMilestone?: { label: string; xpRequired: number; description?: string };
};

function PredictionTable({ rows, loading }: { rows: PredictionRow[]; loading: boolean }) {
    ...
    return (
      <Card className="h-full border-dashed">
        ...
        <CardContent className="text-sm text-muted-foreground">
          Once performance reviews sync from Supabase, AI will highlight potential risks and trajectory guidance here.
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle>Forecasted Goals</CardTitle>
        <CardDescription>Top opportunities identified by AI this week.</CardDescription>
      </CardHeader>
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <span className={`font-semibold ${row.trendClass}`}>{Math.round(row.progress)}%</span>
                </TableCell>
                <TableCell>{row.forecast}</TableCell>
                <TableCell>{row.confidence ? `${row.confidence}%` : 'n/a'}</TableCell>
                <TableCell>{row.reviewDate ? new Date(row.reviewDate).toLocaleDateString() : '—'}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
...
function buildSummaryMetrics(dataset: PerformanceDataset | null, radar: PerformanceRadarMetric[] | undefined, employeesCount: number): SummaryMetric[] {
...
function buildPredictionRows(goalReviews: PerformanceGoalReview[]): PredictionRow[] {
  return goalReviews.slice(0, 5).map((review) => {
    ...
    const confidence = typeof review.score === 'number' ? Math.round((review.score / 5) * 100) : null;
    const trendClass =
      progress >= 80
        ? 'text-emerald-600'
        : progress >= 60
...
function buildRecommendations(goalReviews: PerformanceGoalReview[]): Recommendation[] {
...
function buildXpTrajectory(goalSummary: PerformanceDataset['goalSummary'] | undefined): Trajectory {
  if (!goalSummary) {
    return {
      totalXp: 0,
      nextMilestone: { label: 'First milestone', xpRequired: 1000, description: 'Complete goals and reviews to earn XP.' },
    };
  }
...
const lastReviewLabel = (goalReviews: PerformanceGoalReview[]): string | null => {
  ...
  return recent?.reviewDate ? formatDistanceToNow(new Date(recent.reviewDate), { addSuffix: true }) : null;
};

export function AIInsightsPage() {
  const { dataset, radar = [], goalReviews = [], loading, error } = usePerformanceOverview();
  ...
  return (
    <div className="space-y-6 p-6">
      <InsightsHeader loading={loading} employeesTracked={employeesCount} lastRefreshLabel={lastUpdatedLabel} />
