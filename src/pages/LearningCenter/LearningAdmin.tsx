            ...
            <TableBody>
              {enrollments.slice(0, 20).map((enrollment) => (
                <TableRow key={enrollment.id}>
                  <TableCell className="font-medium">{enrollment.courseId}</TableCell>
                  <TableCell className="capitalize">{enrollment.status.replace('_', ' ')}</TableCell>
                  <TableCell>{Math.round(enrollment.progressPercent)}%</TableCell>
