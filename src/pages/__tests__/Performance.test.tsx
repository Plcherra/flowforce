...
vi.mock('@/stores/useLeaderboardInsights', () => ({
  __esModule: true,
  useLeaderboardInsightsStore: (selector: (state: typeof mockLeaderboardState) => unknown) =>
...
describe('Performance page rendering', () => {
  ...
  it('renders employee performance metrics as cards', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Performance Management' })).toBeInTheDocument();
    ...
    const completedGoalsTile = screen.getByText('Completed Goals').parentElement;
    expect(completedGoalsTile).toBeTruthy();
    expect(completedGoalsTile).toHaveTextContent('1');

    const avgGoalProgressTile = screen.getByText('Avg Goal Progress').parentElement;
    expect(avgGoalProgressTile).toBeTruthy();
    expect(avgGoalProgressTile).toHaveTextContent('65%');

    const avgReviewScoreTile = screen.getByText('Avg Review Score').parentElement;
