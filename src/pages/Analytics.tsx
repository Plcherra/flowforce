...
export default function Analytics() {
  ...
  const [formSummaries, setFormSummaries] = useState<Record<string, { submissionCount: number; completionRate: number }>>({});
  const [formContext, setFormContext] = useState<AssistantContext | null>(null);
  const [reportsContext, setReportsContext] = useState<AssistantContext | null>(null);
  const [assistantContext, setAssistantContext] = useState<AssistantContext | null>(null);
  const [businessContext, setBusinessContext] = useState<AssistantContext | null>(null);
  ...
  const handleSummaryChange = useCallback(
    ({ formId, submissionCount, completionRate }: { formId: string; submissionCount: number; completionRate: number; fieldData: unknown }) => {
  ...
  return (
    <div className={isMobile ? 'space-y-4' : 'space-y-6'}>
      <div className={isMobile ? 'px-4 py-3 space-y-2' : 'flex items-center justify-between'}>
        <div>
          <h1 className={isMobile ? 'text-2xl font-bold tracking-tight' : 'text-3xl font-bold tracking-tight'}>
            Analytics &amp; AI
          </h1>
          <p className="text-muted-foreground">
            Consolidate forms and internal reports, surface AI predictions, and trigger workflow actions in one place.
          </p>
        </div>
      </div>

      <div className={isMobile ? 'px-4' : ''}>
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as TabValue)} className={isMobile ? 'space-y-4' : 'space-y-6'}>
          <TabsList className={`grid w-full ${isMobile ? 'grid-cols-1 h-auto flex-col space-y-1' : 'grid-cols-4'}`}>
            <TabsTrigger value="business" className={isMobile ? 'w-full justify-start' : 'flex items-center gap-2'}>
            ...
            <TabsTrigger value="reports" className={isMobile ? 'w-full justify-start' : 'flex items-center gap-2'}>
          ...
          <TabsContent value="business" className="space-y-6">
            <BusinessAnalyticsBoard companyId={company?.id} onContextChange={setBusinessContext} />
          </TabsContent>
      ...
      <FloatingAssistant context={assistantContext} onTriggerAction={handleCopilotAction} />
    </div>
  );
}
