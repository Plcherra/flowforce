...
import { usePerformanceOverview, type PerformanceGoalSummary, type PerformanceReviewEntry } from '@/hooks/usePerformanceOverview';
...
import { GoalProgressSection, type GoalSummaryCard } from './GoalProgressSection';
...
function buildKpis({
  ...
  const activeGoals = employees.reduce((sum, employee) => sum + employee.activeGoals, 0);
  const completedGoals = employees.reduce((sum, employee) => sum + employee.completedGoals, 0);
...
function buildLeaderboardSnapshot(entries: ReturnType<typeof useLeaderboardData>['entries']): XpTrendEntry[] {
...
function LatestReviewsCard({ reviews, loading }: { reviews: PerformanceReviewEntry[]; loading: boolean }) {
        ...
        {reviews.slice(0, 5).map((review) => (
          <div key={review.id} className="flex items-center gap-3">
            <Avatar className="h-10 w-10">
              ...
              <AvatarFallback>{review.employeeName.slice(0, 2).toUpperCase()}</AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <p className="text-sm font-semibold">{review.employeeName}</p>
              <p className="text-xs text-muted-foreground">
                {review.summary ?? 'No summary provided'} · {new Date(review.date).toLocaleDateString()}
              </p>
            </div>
            <span className="text-sm font-semibold">{review.score?.toFixed(1) ?? 'n/a'}</span>
          </div>
        ))}
      </CardContent>
    </Card>
...
export function PerformancePage() {
  ...
  const { recognitions, loading: recognitionsLoading, error: recognitionsError, refresh: refreshRecognitions } = useRecognitions();
  ...
  const kpis = buildKpis({ employees, reviews, recognitionsCount: recognitions.length });
  const goalCards = buildGoalCards(goals);
  const xpSnapshot = buildLeaderboardSnapshot(entries);
  const xpBySource = analytics?.xpBySource ?? { tasks: 0, goals: 0, recognitions: 0, training: 0 };
  ...
  const combinedLoading = performanceLoading || recognitionsLoading || leaderboardLoading;
  ...
  const combinedError = performanceError ?? recognitionsError ?? leaderboardError ?? null;

  return (
    <div className="space-y-6 p-6">
      <PerformanceHeader loading={combinedLoading} onRefresh={handleRefresh} lastUpdatedLabel={lastUpdatedLabel} />
      ...
      <div className="grid gap-6 lg:grid-cols-[1.6fr,1fr]">
        ...
        <XPTrendsSection xpBySource={xpBySource} leaderboard={xpSnapshot} loading={leaderboardLoading} />
      </div>
