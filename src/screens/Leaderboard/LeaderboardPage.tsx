...
function deriveMilestones(totalXp: number) {
  const nextMilestone =
    LEADERBOARD_MILESTONES.find((milestone) => totalXp < milestone.xpRequired) ??
    {
  ...
  const previousMilestone =
    [...LEADERBOARD_MILESTONES].reverse().find((milestone) => milestone.xpRequired <= totalXp) ?? undefined;
...
function LeaderboardStats({
  ...
  averageXp: number;
  topDepartment?: { name: string | null; participantCount: number; totalXp: number } | null;
}) {
  return (
      ...
      <CardContent className="space-y-3">
        ...
        <div>
          ...
          <p className="text-sm font-semibold">
            {topDepartment?.name ?? 'None'} · {topDepartment?.participantCount ?? 0} participant
...
export function LeaderboardPage() {
  ...
  const { entries, analytics, loading, error, refresh } = useLeaderboardData(period);
