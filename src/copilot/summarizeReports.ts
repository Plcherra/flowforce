...
export async function summarizeWeeklyReports(
  ...
  try {
    ...
    if (error) {
      throw new Error(
        error.message ?? "Failed to summarize weekly reports via RPC.",
      );
    }
    ...
    logger.error(
      "[summarizeWeeklyReports] Supabase function invocation failed",
      { error: rpcError, tags: ["error"] },
    );
    throw rpcError;
  }
}
