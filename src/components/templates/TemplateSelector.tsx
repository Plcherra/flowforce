...
export default function TemplateSelector({ onTemplateSelect, selectedTemplate }: TemplateSelectorProps) {
  ...
  const loadMoreTemplates = useCallback(() => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      setVisibleTemplates(prev => Math.min(prev + 4, BUSINESS_TEMPLATES.length));
      setIsLoading(false);
    }, 300);
  }, [visibleTemplates]);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && visibleTemplates < BUSINESS_TEMPLATES.length) {
          loadMoreTemplates();
        }
      },
      { threshold: 0.1 }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-2">Choose Your Business Template</h3>
        <p className="text-gray-600 dark:text-gray-400">
          Select a pre-configured template that matches your business type. You can customize sections later.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-lg">{t('onboarding.customTemplate.title')}</CardTitle>
        ...
        {BUSINESS_TEMPLATES.slice(0, visibleTemplates).map((template, index) => {
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <CardTitle className="text-lg">{getLocalizedTemplate(template).name}</CardTitle>
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    <AccordionItem value="sections" className="border-none">
                      <AccordionTrigger className="py-2 text-sm font-medium hover:no-underline">
                        <div className="flex items-center space-x-2">
                          <span>Included Sections ({template.sections.length})</span>
                        </div>
                      </AccordionTrigger>
                    ...
                    {template.defaultRoles && template.defaultRoles.length > 0 && (
                      <AccordionItem value="roles" className="border-none">
                        <AccordionTrigger className="py-2 text-sm font-medium hover:no-underline">
                          <div className="flex items-center space-x-2">
                            <span>Default Roles ({template.defaultRoles.length})</span>
                          </div>
                        </AccordionTrigger>
                        <AccordionContent className="pt-2">
                          <div className="flex flex-wrap gap-1">
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <span className="text-green-800 dark:text-green-200 font-medium">
              Template Selected: {BUSINESS_TEMPLATES.find(t => t.id === selectedTemplateId)?.name}
            </span>
          </div>
        </motion.div>
      )}
