...
export function ClosedLoopSummary({ loading, error, state, onRefresh }: ClosedLoopSummaryProps) {
  const severity = (state?.detection.severityBreakdown ?? { high: 0, medium: 0, low: 0 }) as Record<'high' | 'medium' | 'low', number>;
  ...
  const aiSummary = state?.interpretation.summary ?? 'AI insights not yet generated.';
  const trimmedSummary = aiSummary.length > 180 ? `${aiSummary.slice(0, 177)}…` : aiSummary;
        ...
        {loading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <section className="rounded-lg border bg-muted/30 p-4">
              ...
              <p className="mt-3 text-sm text-muted-foreground">{state.detection.summary}</p>
            ...
            <section className="rounded-lg border bg-muted/30 p-4">
              ...
              <p className="mt-3 text-xs uppercase text-muted-foreground">Risk score</p>
              <Progress value={Math.min(100, riskScore)} className="mt-1" />
              <p className="mt-2 text-sm font-semibold text-foreground">{riskScore}/100</p>
              <p className="mt-2 text-xs text-muted-foreground">{trimmedSummary}</p>
              {themes.length > 0 && (
                <p className="mt-3 text-xs text-muted-foreground">
                  Themes:{' '}
                  <span className="font-medium text-foreground">{themes.slice(0, 3).join(', ')}</span>
                </p>
              )}
            </section>
            ...
            <section className="rounded-lg border bg-muted/30 p-4">
              ...
              <div className="mt-3 space-y-2 text-sm">
                <p className="flex items-center justify-between">
                  Pending approvals
                  <span className="font-semibold text-foreground">{state.approvals.pending.length}</span>
                </p>
                <p className="flex items-center justify-between text-muted-foreground">
                  Ready to automate
                  <span className="font-medium text-foreground/80">{state.approvals.readyToAutomate.length}</span>
                </p>
                <p className="flex items-center justify-between text-muted-foreground">
                  Manual queue
                  <span className="font-medium text-foreground/80">{state.approvals.awaitingHuman.length}</span>
                </p>
              </div>
              <p className="mt-3 text-xs text-muted-foreground">{state.approvals.summary}</p>
            </section>
            ...
            <section className="rounded-lg border bg-muted/30 p-4">
              ...
              <Badge
                ...
                className={cn(
                  ...
                  guardrailStatus === 'allowed' && 'border-emerald-500/50 text-emerald-600',
                  guardrailStatus === 'warning' && 'border-amber-500/50 text-amber-600',
                  guardrailStatus === 'blocked' && 'border-destructive/60 text-destructive',
                )}
              >
                {guardrailStatus}
              </Badge>
              <p className="mt-3 text-sm text-muted-foreground">{guardrailMessage}</p>
            ...
            <section className="rounded-lg border bg-muted/30 p-4">
              ...
              <p className="mt-3 text-sm text-muted-foreground">{state.learning.summary}</p>
              <div className="mt-4 space-y-2 text-xs text-muted-foreground">
                ...
                <p>
                  Tasks completed:{' '}
                  <span className="font-medium text-foreground">{state.learning.completedTasks}</span>
                </p>
                <p>
                  Critical signals resolved:{' '}
                  <span className="font-medium text-foreground">{state.learning.resolvedEvents}</span>
                </p>
              </div>
            </section>
          </div>
