...
export function useOnboardingPerformance({
  ...
  }, [selectedTemplate, enabledSections.length, customRoles.length, positions.length]);

  // Memoize validation functions
  const isStepValid = useCallback((step: number, userInfo: UserMetadata, companyInfo: Record<string, unknown>) => {
    switch (step) {
      case 1:
        return Boolean(
          userInfo.firstName &&
