...
export default function InviteSignUpForm({ onSubmit, isLoading }: InviteSignUpFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSubmit(inviteData.email, password, inviteData.firstName, inviteData.lastName);
  };
      ...
      <CardContent>
        ...
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className="font-medium text-green-800">Your Profile Information</h4>
          </div>
          <div className="space-y-2 text-sm text-green-700">
            ...
            {inviteData.phone && <p><strong>Phone:</strong> {inviteData.phone}</p>}
            {inviteData.birthDate && <p><strong>Birth Date:</strong> {new Date(inviteData.birthDate).toLocaleDateString()}</p>}
          </div>
        </div>
