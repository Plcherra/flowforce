...
import { getLockStateForWeek, allowEdit, computeAutoLockThreshold, startOfIsoWeek } from '@/availability/lockEngine';
import type { LockEngineDeps } from '@/availability/lockEngine';
...
const hashGrid = (grid: AvailabilityGrid) => JSON.stringify(grid, Object.keys(grid).sort());
...
export function AvailabilityRequestForm({
  ...
  const [editingGrid, setEditingGrid] = useState<AvailabilityGrid>(() => cloneGrid(initialAvailability));
  ...
  const { data: lockInfo } = useQuery<LockInfo>({
    ...
    queryFn: async () => {
      ...
      const prefs: OrgPrefs | null = prefData
        ? {
            id: prefData.id,
            availabilityLockMode: prefData.availability_lock_mode as AvailabilityLockMode,
  ...
  const { data: allowMap } = useQuery<Record<string, boolean>>({
    ...
    queryFn: async () => {
      if (!lockInfo) return {};
      const dates = DAY_LABELS.map((_, index) => getDateForOffset(weekStart, index).format('YYYY-MM-DD'));
  ...
  const { data: lastRequest } = useQuery<AvailabilityRequest | null>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, created_at, status, week_start, manager_id, decision_note, payload')
  ...
  const statusBadge = useMemo(() => {
    ...
    return (
      <Badge variant={variant}>
        Last request: {lastRequest.status}
      </Badge>
    );
  }, [lastRequest]);
    ...
    <Card className="border bg-background shadow-sm">
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Manage your weekly availability. Locked weeks require a manager review.
            </CardDescription>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {lockInfo?.state !== 'open' ? <Lock className="h-3 w-3" /> : <Unlock className="h-3 w-3" />}
      ...
      <CardContent className="space-y-4">
        <div
          className={cn(
            'rounded-lg border',
            directEditAllowed ? 'border-border' : 'border-dashed border-destructive/50 bg-destructive/5',
          )}
        >
                ...
                {HOURS.map((hour) => (
                  <tr key={hour}>
                    ...
                    {DAY_LABELS.map((_, dayIndex) => {
                      const dateISO = getDateForOffset(weekStart, dayIndex).format('YYYY-MM-DD');
                      const isSelected = editingGrid[dayIndex]?.includes(hour);
                      const disabled = !directEditAllowed || (allowMap && allowMap[dateISO] === false);
                      return (
                        <td key={`${dayIndex}-${hour}`} className="px-3 py-1.5">
                          <button
                            ...
                            className={cn(
                              ...
                              isSelected && disabled && 'bg-muted text-muted-foreground',
                            )}
                          >
            ...
            <Dialog open={requestDialogOpen} onOpenChange={setRequestDialogOpen}>
              ...
              <DialogContent className="sm:max-w-xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Provide the timeframe and reason for your change. Managers will review and respond.
                  </DialogDescription>
                </DialogHeader>
                <Tabs defaultValue="custom" className="space-y-4">
                  <TabsList>
                    ...
                    <TabsTrigger value="weekend" onClick={() => applyPreset('weekend')}>
                      Weekend
                    </TabsTrigger>
                    <TabsTrigger value="custom" onClick={() => applyPreset('custom')}>
                      Custom
                    </TabsTrigger>
                  </TabsList>
                    ...
                    <ul className="space-y-1 text-xs">
                      {diffSummary.map((entry) => (
                        <li key={entry.day}>
                          ...
                          {entry.added.length > 0 && entry.removed.length > 0 && <span> · </span>}
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button
                    ...
                    disabled={requestReason.trim().length === 0 || diffSummary.length === 0}
                  >
                    Submit request
                  </Button>
                </div>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <div className="space-y-1.5">
        <Label htmlFor="request-start" className="flex items-center gap-1 text-xs text-muted-foreground">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="request-end" className="flex items-center gap-1 text-xs text-muted-foreground">
          <Calendar className="h-3 w-3" />
          End date
        </Label>
        <Input
