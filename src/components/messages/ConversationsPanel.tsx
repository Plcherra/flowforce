...
import { getConversationName, type Conversation } from '@/components/messages/conversations';
...
export function ConversationsPanel({
  ...
  const renderConversationMeta = (conversation: Conversation) => {
    ...
    const conversationName = getConversationName(conversation, usersById, currentUserId) || 'Conversation';
    const avatarSrc = counterpartId ? usersById.get(counterpartId)?.avatar : undefined;
    ...
    return (
      <div
        ...
        className={cn(
          ...
          activeConversationId === conversation.id && 'border-primary/70 bg-primary/5 shadow-md'
        )}
      >
        <button
          type="button"
        ...
        >
          <Avatar className="h-11 w-11">
            {avatarSrc ? <AvatarImage src={avatarSrc} alt={conversationName} /> : <AvatarFallback>{avatarFallback}</AvatarFallback>}
          </Avatar>
          ...
          {conversation.unreadCount > 0 && (
            <Badge variant="default" className="ml-auto px-2 py-0.5 text-[11px]">
        ...
        <CardContent className="flex flex-1 flex-col gap-3 overflow-hidden">
          ...
          <ScrollArea className="min-h-0 flex-1 pr-1">
            <div className="space-y-3">
              {conversations.map((conversation) => renderConversationMeta(conversation))}
