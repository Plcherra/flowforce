...
import type { EmployeeReport, EmployeeReportSummary, EmployeeReportCategory } from '@/types/people';
  ...
  const reportsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, employee_id, date, category, severity, notes, created_at, created_by, updated_at')
  ...
  const createReportMutation = useMutation({
    mutationFn: async (values: ReportFormValues) => {
      ...
      if (values.attachment) {
        ...
        const { error: uploadError } = await supabase.storage.from('attachments').upload(path, values.attachment);
        if (uploadError) throw uploadError;
        const { data: signed } = supabase.storage.from('attachments').getPublicUrl(path);
      ...
      const insertPayload = {
        ...
        date: values.date,
        notes: values.notes + (attachmentUrl ? `\nAttachment: ${attachmentUrl}` : ''),
        created_by: user.id,
      };

      const { error } = await supabase.from('employee_report').insert(insertPayload);
      if (error) throw error;
    },
    onSuccess: () => {
      toast({
      ...
      <Card className="border bg-background shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormValues((prev) => ({ ...prev, category: value as EmployeeReportCategory }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(value) => setFormValues((prev) => ({ ...prev, severity: Number(value) }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-1">
              <label className="text-sm font-semibold text-muted-foreground">Attachment</label>
              <Input
                type="file"
          ...
          <Button
            ...
            disabled={!formValues.employeeId || !formValues.notes || createReportMutation.isLoading}
          >
            {createReportMutation.isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            Submit report
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="border bg-background shadow-sm">
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-muted-foreground">Weekly summaries</h3>
            <Button
              size="sm"
              variant="outline"
              onClick={() => generateSummaryMutation.mutate(formValues.employeeId || DEFAULT_ORG_ID)}
              disabled={generateSummaryMutation.isLoading || !formValues.employeeId}
            >
              {generateSummaryMutation.isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Generate summary
            </Button>
          </div>
          <div className="space-y-3">
            {(summariesQuery.data ?? []).map((summary) => (
              <div key={`${summary.employee_id}-${summary.week_start}`} className="rounded-lg border p-3">
          ...
          <div className="space-y-3">
            {(reportsQuery.data ?? []).map((report) => {
              const employee = employeesQuery.data?.find((profile) => profile.id === report.employee_id);
              return (
            ...
            {reportsQuery.data?.length === 0 && (
              <div className="flex flex-col items-center gap-2 rounded border border-dashed py-10 text-muted-foreground">
                ...
                <span className="text-sm">No reports yet. Submit a report to populate this inbox.</span>
              </div>
            )}
          </div>
        </CardContent>
...
async function generateWeeklySummary(employeeId: string, weekStart: string): Promise<string> {
  const { data, error } = await supabase
    .from('employee_report')
