      ...
      <DrawerContent className="max-h-[95vh]">
        <div className="flex h-full flex-col">
          <DrawerHeader className="border-b px-6 py-4 text-left">
            <DrawerTitle className="text-xl">{event?.title || 'Event details'}</DrawerTitle>
            <DrawerDescription className="text-sm text-muted-foreground">
              {event ? `${formatDateTime(event.start)} • ${event.type ?? 'event'}` : 'Select an event to inspect details.'}
            </DrawerDescription>
          </DrawerHeader>
                  ...
                  {event ? (
                    <>
                      <section>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">{event.description || 'No description provided.'}</p>
                      </section>
                      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded-md border p-3 text-sm">
                          ...
                          <p className="text-muted-foreground">{formatDateTime(event.end ?? null)}</p>
                        </div>
                        <div className="rounded-md border p-3 text-sm">
                          <p className="font-semibold text-foreground">Location</p>
                          <p className="text-muted-foreground">{event.location || 'Not specified'}</p>
                        </div>
                        <div className="rounded-md border p-3 text-sm">
                          <p className="font-semibold text-foreground">Event type</p>
                          <p className="capitalize text-muted-foreground">{event.type ?? 'event'}</p>
                        </div>
                      </section>
                    </>
                  ) : (
                ...
                <TabsContent value="participants" className="space-y-3">
                  {participants.length === 0 ? (
                    ...
                    participants.map((participant) => (
                      <div key={participant.id} className="flex items-center justify-between rounded-md border bg-card px-3 py-2 text-sm">
                        <div>
                        ...
                        <Badge variant="outline" className="text-[11px]">{participant.responseStatus ?? 'Invited'}</Badge>
                      </div>
                    ))
                  )}
                </TabsContent>
          ...
          <div className="flex items-center justify-between border-t px-6 py-4">
            <div className="text-xs text-muted-foreground">
              {busy ? 'Working…' : event ? `Created ${formatDateTime(event.raw.created_at ?? undefined)}` : 'Idle'}
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => onOpenChange(false)} disabled={busy}>
                Close
              </Button>
              <Button variant="destructive" size="sm" onClick={handleDelete} disabled={!eventId || busy}>
                Delete
              </Button>
            </div>
          </div>
...
const timeRange = (shift?: { start_time?: string; end_time?: string } | null) => {
  if (!shift?.start_time || !shift?.end_time) return 'No time set';
  const start = parseISO(shift.start_time);
  const end = parseISO(shift.end_time);
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return 'No time set';
