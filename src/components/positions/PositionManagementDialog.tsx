...
import { Plus, Edit2, Trash2, Users, Palette } from 'lucide-react';
...
export function PositionManagementDialog({ children }: PositionManagementDialogProps) {
  const { positions, loading, createPosition, updatePosition, deletePosition, getPositionUsers } = usePositions();
  ...
  const handleDelete = async (position: Position) => {
    if (confirm(`Are you sure you want to delete the "${position.name}" position?`)) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">Position Management</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label htmlFor="role" className="text-sm">Role Level</Label>
                <select
                  id="role"
                  value={formData.role}
                  onChange={(e) => setFormData(prev => ({ ...prev, role: e.target.value as 'staff' | 'supervisor' | 'manager' | 'admin' }))}
          ...
          {/* Positions List */}
          <div className="space-y-4">
            <h3 className="text-base sm:text-lg font-semibold">Existing Positions</h3>
            
            {loading ? (
              <div className="text-sm text-muted-foreground">Loading positions...</div>
            ) : positions.length === 0 ? (
              <div className="text-sm text-muted-foreground">No positions created yet</div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="font-medium text-sm sm:text-base truncate">{position.name}</div>
