...
import { 
  GripVertical, Trash2, FileText, AlignLeft, Hash, ChevronDown, 
  CheckCircle, Image, Calculator, ScanLine, Calendar, MapPin, ImageUp, 
  Mic, Video, CheckSquare, Upload, Star, PenTool, Sliders, Plus,
  LayoutGrid, List, Edit
} from 'lucide-react';
...
const fieldTypes = [
  ...
  { value: 'select', label: 'Dropdown', icon: 'ChevronDown', category: 'basic' },
  { value: 'number', label: 'Number', icon: 'Hash', category: 'basic' },
  { value: 'textarea', label: 'Open ended', icon: 'AlignLeft', category: 'basic' },
  { value: 'yes_no', label: 'Yes/No', icon: 'CheckCircle', category: 'basic' },
  { value: 'date', label: 'Date', icon: 'Calendar', category: 'basic' },
  { value: 'number_slider', label: 'Number Slider', icon: 'Sliders', category: 'basic' },
  { value: 'location', label: 'Location', icon: 'MapPin', category: 'location' },
  { value: 'image_upload', label: 'Image Upload', icon: 'ImageUp', category: 'media' },
  { value: 'audio_recording', label: 'Audio Recording', icon: 'Mic', category: 'media' },
  { value: 'video_upload', label: 'Video Upload', icon: 'Video', category: 'media' },
  { value: 'file', label: 'File upload', icon: 'Upload', category: 'media' },
  { value: 'scanner', label: 'Scanner', icon: 'ScanLine', category: 'interactive' },
  { value: 'image_selection', label: 'Image selection', icon: 'Image', category: 'interactive' },
  { value: 'signature', label: 'Signature', icon: 'PenTool', category: 'interactive' },
  { value: 'rating', label: 'Rating', icon: 'Star', category: 'interactive' },
  { value: 'task', label: 'Task', icon: 'CheckSquare', category: 'interactive' },
] as const;
...
const getFieldIcon = (fieldType: string) => {
  ...
  const iconMap: Record<string, any> = {
    FileText, AlignLeft, Hash, ChevronDown, CheckCircle, Image, Calculator,
    ScanLine, Calendar, MapPin, ImageUp, Mic, Video, CheckSquare, Upload,
    Star, PenTool, Sliders
  };
...
export default function FieldEditor({ fields, onUpdateField, onRemoveField, onMoveField }: FieldEditorProps) {
        ...
        <div className="flex-1 space-y-3">
                ...
                <CardContent className="space-y-3">
                  ...
                  {field.field_type === 'description' ? (
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => onUpdateField(index, { field_type: value as FormFieldType })}
                          >
                      ...
                      {field.field_type !== 'formula' && field.field_type !== 'number_slider' && field.field_type !== 'yes_no' && (
                        <div>
                      ...
                      {needsOptions(field.field_type) && (
                        <DropdownOptionsEditor
                          options={field.options || []}
                          allowMultiple={field.allow_multiple_selection || false}
                          onUpdateOptions={(options) => onUpdateField(index, { options })}
                          onUpdateMultiple={(allow) => onUpdateField(index, { allow_multiple_selection: allow })}
                        />
                      )}
                      ...
                      {field.field_type === 'formula' && (
                        <div>
                          ...
                          <FormulaField
                            ...
                            availableFields={fields.filter(f => f.field_type === 'number').map(f => ({ 
                              label: f.label, 
                              name: f.label.toLowerCase().replace(/\s+/g, '_'), 
                              type: 'number' 
                            }))}
                            isBuilding={true}
                            onFormulaChange={(formula) => onUpdateField(index, { formula_expression: formula })}
                            className="mt-2"
                          />
                        </div>
                      )}
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => onUpdateField(index, { min_value: Number(e.target.value) })}
                              className="text-sm"
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => onUpdateField(index, { max_value: Number(e.target.value) })}
                              className="text-sm"
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => onUpdateField(index, { step_value: Number(e.target.value) })}
                              className="text-sm"
                            />
                          </div>
                        </div>
                  ...
                  <div className="border-t pt-4 mt-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <span className="text-sm">Show this field only if...</span>
                      </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            >
                              ...
                              <SelectContent>
                                {getAvailableFieldsForConditions(index).map((availableField) => (
                          ...
                          <div>
                            ...
                            <Select
                              value={field.conditional_logic.condition_type || 'equals'}
                              onValueChange={(value) => onUpdateField(index, { 
                                conditional_logic: { 
                                  ...field.conditional_logic,
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="any_of">is any of</SelectItem>
                                <SelectItem value="none_of">is none of</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                        </div>
            ...
            <Card className="border-2 border-primary">
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle className="text-base">Edit Field: {fields[expandedField]?.label || `Field ${expandedField + 1}`}</CardTitle>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => onUpdateField(expandedField, { label: e.target.value })}
                          placeholder="Enter field label"
                          className="text-sm"
                        />
                      </div>
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => onUpdateField(expandedField, { field_type: value as FormFieldType })}
                        >
                          <SelectTrigger className="text-sm">
                            <SelectValue />
                          </SelectTrigger>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => onUpdateField(expandedField, { description: e.target.value })}
                        placeholder="Help text for this field"
                        className="text-sm"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => onUpdateField(expandedField, { placeholder: e.target.value })}
                        placeholder="Placeholder text"
                        className="text-sm"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <label className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => 
                            onUpdateField(expandedField, { is_required: !!checked })
                          }
                        />
                        <span className="text-sm font-medium">Required</span>
                      </label>
                    ...
                    {needsOptions(fields[expandedField]?.field_type || '') && (
                      <DropdownOptionsEditor
                        ...
                        allowMultiple={fields[expandedField]?.allow_multiple_selection || false}
                        onUpdateOptions={(options) => onUpdateField(expandedField, { options })}
                        onUpdateMultiple={(allow) => onUpdateField(expandedField, { allow_multiple_selection: allow })}
                      />
                    )}
                  </>
                )}
          ...
          {fields.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Click "Add field" or select elements from the left to start building your form</p>
            </div>
          )}
        </div>
      </div>
