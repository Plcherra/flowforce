...
export default function FormPreview({ formTitle, formDescription, fields }: FormPreviewProps) {
  const isFieldVisible = (field: FormField, fieldIndex: number) => {
    if (!field.conditional_logic?.enabled) return true;
    
    const { field_id, condition_type, condition_values = [] } = field.conditional_logic;
    ...
    if (referencedFieldIndex < 0 || referencedFieldIndex >= fieldIndex) return true;
      ...
      case 'phone':
        return (
          <div className={commonProps.className}>
            ...
            {commonProps.description && <p className="text-sm text-muted-foreground mt-1">{commonProps.description}</p>}
            <Input 
      ...
      case 'textarea':
        return (
          <div className={commonProps.className}>
            <label className="text-sm font-medium">{commonProps.label}</label>
            {commonProps.description && <p className="text-sm text-muted-foreground mt-1">{commonProps.description}</p>}
            <Textarea placeholder={field.placeholder || "Enter your response..."} className="mt-2" />
          </div>
        );
      case 'number':
        return (
          <div className={commonProps.className}>
            <label className="text-sm font-medium">{commonProps.label}</label>
            {commonProps.description && <p className="text-sm text-muted-foreground mt-1">{commonProps.description}</p>}
            <Input type="number" placeholder={field.placeholder || "Enter a number"} className="mt-2" />
          </div>
        );
      case 'select':
        return (
            ...
            <div className="mt-3 space-y-3">
              {field.allow_multiple_selection ? (
                ...
                field.options?.map((option, i) => (
                  <div key={i} className="flex items-center space-x-2 p-2 rounded-md hover:bg-muted/50 transition-colors">
                ...
                <RadioGroup defaultValue="" className="space-y-2">
                  {field.options?.map((option, i) => (
                    <div key={i} className="flex items-center space-x-2 p-2 rounded-md hover:bg-muted/50 transition-colors">
                      <RadioGroupItem value={option} id={`field-${index}-option-${i}`} />
                      <Label 
                        htmlFor={`field-${index}-option-${i}`} 
                        className="text-sm cursor-pointer flex-1"
                      >
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          {fields.map((field, index) => (
            <div key={index} className={isFieldVisible(field, index) ? '' : 'opacity-50'}>
          ...
          {fields.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Preview will appear here as you add fields</p>
            </div>
          )}
        </div>
      </div>
