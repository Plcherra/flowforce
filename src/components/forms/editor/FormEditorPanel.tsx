...
export function FormEditorPanel() {
  ...
  const setActiveSection = useFormSchemaStore((state) => state.setActiveSection);
  ...
  const activeSection = useMemo(
    () => schema?.sections.find((section) => section.id === activeSectionId) ?? schema?.sections[0],
    [schema, activeSectionId],
  );

  const activeField = useMemo(() => {
    if (!activeSection) return undefined;
    return activeSection.fields.find((field) => field.id === activeFieldId) ?? activeSection.fields[0];
  ...
  const allFields = useMemo(
    () => schema?.sections.flatMap((section) => section.fields.map((field) => ({ id: field.id, label: field.label }))) ?? [],
    [schema],
  );
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    const templateJSON = event.dataTransfer.getData('application/form-field-template');
    if (!templateJSON || !schema) return;
    const template = JSON.parse(templateJSON) as { type: FormFieldType; label: string };
      ...
      <div className="sticky top-0 z-10 border-b bg-background/95 p-4 backdrop-blur">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-3">
            ...
            <Button variant="outline" size="sm" onClick={() => addSection('Untitled section')}>
              + Add section
            </Button>
          </div>
          <div className="ml-auto flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <label className="flex items-center gap-2">
              <Switch
                checked={ownerOnly}
                onCheckedChange={(checked) => setMetadata({ ...metadata, ownerOnly: checked })}
                aria-label="Restrict form to owner"
              />
              <span>Owner only</span>
            </label>
            <span className="hidden md:inline">Drag a field here or use quick add:</span>
            <Button size="sm" variant="ghost" onClick={() => handleAddQuickField('text')}>
              Text
            </Button>
            <Button size="sm" variant="ghost" onClick={() => handleAddQuickField('select')}>
              Dropdown
            </Button>
            <Button size="sm" variant="ghost" onClick={() => handleAddQuickField('checkbox')}>
              Checkbox
            </Button>
          </div>
        </div>
            ...
            <div key={section.id} className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className={cn(
                    'h-7 text-xs',
                    activeSection?.id === section.id ? 'text-primary' : 'text-muted-foreground',
                  )}
                >
                  Focus section
                </Button>
                ...
                {section.fields.map((field) => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeField?.id === field.id ? 'border-primary bg-primary/10' : 'border-transparent',
                    )}
                  >
                    <p className="text-sm font-medium">{field.label || 'Untitled field'}</p>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  updateField(activeSection.id, activeField.id, { label: event.target.value })
                }
              />
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    updateField(activeSection.id, activeField.id, { type: value as FormFieldType })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateField(activeSection.id, activeField.id, { placeholder: event.target.value })
                  }
                />
              </div>
            </div>
            <div className="flex items-center justify-between rounded-lg border bg-muted/40 p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Users must complete this field before submitting.</p>
              </div>
              <Switch
                checked={Boolean(activeField.required)}
                onCheckedChange={(checked) =>
                  updateField(activeSection.id, activeField.id, { required: checked })
                }
              />
            </div>
      ...
      <div className="border-t bg-background p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Use formulas to compute values automatically.</p>
          </div>
        ...
        <div className="mt-4 space-y-3">
          {formulas.length === 0 && (
            <p className="text-xs text-muted-foreground">No formulas configured yet.</p>
          )}
          {formulas.map((formula) => (
            <div key={formula.id} className="space-y-3 rounded-lg border bg-muted/40 p-3">
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold uppercase text-muted-foreground">Target field</label>
                  <Select
                    value={formula.targetFieldId}
                    onValueChange={(value) => upsertFormula({ ...formula, targetFieldId: value })}
                  >
                    <SelectTrigger>
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold uppercase text-muted-foreground">Expression</label>
                <Textarea
                  value={formula.expression}
                  onChange={(event) => upsertFormula({ ...formula, expression: event.target.value })}
