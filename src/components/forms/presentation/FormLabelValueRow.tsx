...
export function FormLabelValueRow({
  label,
  value,
  emphasis = "none",
  className,
}: FormLabelValueRowProps) {
  const empty = isEmptyValue(value);

  return (
    <div
      className={cn(
        "flex flex-col gap-1 border-b border-border/50 pb-3 last:border-b-0 last:pb-0",
        className,
      )}
    >
      ...
      {empty ? (
        <div
          className="h-6 rounded-md border border-dashed border-border/60 bg-muted/20"
          aria-hidden="true"
        />
      ) : (
        <div
