...
export function DesignContentStep({
  ...
  const [previewMedia, setPreviewMedia] = useState<UpdateMediaItem | null>(null);
  const [isDragging, setIsDragging] = useState(false);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const { uploadFiles, isUploading, progress, error: uploadError } = useUploadMedia();
  ...
  const handleMediaUpload = async (files?: FileList | File[]) => {
    ...
    if (uploaded.length > 0) {
      updateFormData({ updateMedia: [...(formData.updateMedia ?? []), ...uploaded] });
    }
  ...
  const charactersRemaining = Math.max(0, MAX_BODY_CHARACTERS - bodyPlainText.length);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        <div className="space-y-6">
          <section id="wizard-section-content" className="rounded-2xl border bg-card p-6 shadow-sm">
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 text-muted-foreground">
                <TypeIcon className="h-4 w-4" />
                <span className="text-sm font-medium uppercase tracking-wide">Content</span>
              </div>
            ...
            <div className="mt-6 space-y-5">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => updateFormData({ title: limitTitleLines(e.target.value) })}
                ...
                <p className="text-xs text-muted-foreground">
                  {formData.title.length}/{MAX_TITLE_CHARACTERS} characters · limited to two lines
                </p>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => updateFormData({ priority: value as WizardFormData['priority'] })}
                  >
                    <SelectTrigger className="rounded-xl">
              ...
              <div className="space-y-3">
                ...
                <RichTextEditor value={formData.body} onChange={handleContentChange} />
              </div>
            </div>
          </section>

          <section id="wizard-section-media" className="rounded-2xl border bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ImageIcon className="h-4 w-4 text-primary" />
                <div>
                  <h4 className="text-sm font-semibold">Media & Attachments</h4>
                  <p className="text-xs text-muted-foreground">Add visuals, video or files to your announcement.</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <Button variant="outline" size="sm" className="mt-3 rounded-full" onClick={handleBrowseClick}>
                Browse library
              </Button>
              <p className="mt-2 text-xs text-muted-foreground">Images, video, PDF or Docs — up to 50MB each</p>
              {isUploading && (
              ...
              <input
                ...
                onChange={(event) => handleMediaUpload(event.target.files ?? undefined)}
                className="hidden"
              />
            </div>
            ...
            {mediaItems.length > 0 && (
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {mediaItems.map((media) => (
                  ...
                  >
                    {media.type === 'image' ? (
                      <img src={media.url} alt={media.name} className="h-32 w-full rounded-xl object-cover" />
                    ) : media.type === 'video' ? (
                      ...
                      <div className="flex h-32 w-full flex-col items-center justify-center rounded-xl bg-background/70 text-xs text-muted-foreground">
                        ...
                        <span className="mt-2 line-clamp-2 px-2 text-center">{media.name}</span>
                      </div>
                    )}
                    <div className="mt-2">
                      <p className="line-clamp-1 text-sm font-medium">{media.name}</p>
                      <p className="text-xs text-muted-foreground">{(media.size / (1024 * 1024)).toFixed(1)} MB</p>
                    </div>
                    <Button
                      type="button"
                      size="icon"
                      ...
                      onClick={(event) => {
                        ...
                        updateFormData({
                          updateMedia: mediaItems.filter((item) => item.id !== media.id),
                        });
                      }}
                    >
          ...
          <section id="wizard-section-style" className="rounded-2xl border bg-card p-6 shadow-sm">
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-primary" />
              <div>
                <h4 className="text-sm font-semibold">Visual Style</h4>
                <p className="text-xs text-muted-foreground">Tune background, gradients, and patterns.</p>
              </div>
            </div>
                ...
                {GRADIENT_PRESETS.map((preset) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Gradient preset</p>
                    </div>
                  </button>
                ))}
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(['solid', 'gradient', 'pattern'] as const).map((style) => (
                    <Button
                      key={style}
                      type="button"
                      variant={formData.backgroundStyle.type === style ? 'default' : 'outline'}
      ...
      <Dialog open={Boolean(previewMedia)} onOpenChange={(open) => !open && setPreviewMedia(null)}>
        <DialogContent className="max-w-3xl">
          ...
          {previewMedia?.type === 'image' && (
            <img src={previewMedia.url} alt={previewMedia.name} className="w-full rounded-xl object-contain" />
          )}
          {previewMedia?.type === 'video' && (
            <video controls src={previewMedia.url} className="w-full rounded-xl" />
          )}
          {previewMedia?.type === 'file' && (
            <div className="flex flex-col items-center gap-4 py-6 text-center">
              <FileText className="h-10 w-10 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">Download to view this document.</p>
