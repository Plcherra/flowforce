...
export function CourseModulesForm({ onAdd }: CourseModulesFormProps) {
  ...
  const handleAdd = () => {
    ...
    const payload: CourseModuleInput = {
      ...
      estimatedMinutes: Math.max(5, Number.isFinite(newModule.duration) ? newModule.duration : 0),
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="space-y-2 md:col-span-2">
          ...
          <Select
            ...
            onValueChange={(value) => setNewModule((prev) => ({ ...prev, type: value }))}
          >
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(event) =>
              setNewModule((prev) => ({ ...prev, duration: Number(event.target.value) }))
            }
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            value={newModule.xp}
            onChange={(event) =>
              setNewModule((prev) => ({ ...prev, xp: Number(event.target.value) }))
            }
          />
        </div>
      </div>
          ...
          {newModule.assets.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {newModule.assets.map((asset) => (
                <Badge key={asset.id} variant="secondary" className="flex items-center gap-2">
