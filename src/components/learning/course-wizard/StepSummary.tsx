  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        ...
        <CardDescription>Double-check the experience before deploying</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="rounded-xl border bg-muted/20 p-4 text-sm">
          ...
          <p className="text-muted-foreground">
            Target roles: {targetRoles.length > 0 ? targetRoles.join(', ') : 'All employees'}
          </p>
        </div>
        <div>
          <p className="text-sm font-semibold text-muted-foreground">Modules ({modules.length})</p>
          <ul className="mt-2 space-y-2 text-sm">
            {modules.map((module, index) => (
              <li key={`${module.title}-${index}`} className="rounded-lg border bg-muted/20 p-3">
          ...
          {modules.length === 0 && (
            <p className="text-sm text-muted-foreground">Add at least one module to launch the course.</p>
          )}
        </div>
      </CardContent>
    </Card>
