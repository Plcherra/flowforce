...
export function InventoryRecipeDialog({ item, open, onOpenChange, trigger }: InventoryRecipeDialogProps) {
  ...
  const handleAddIngredient = async () => {
    if (!formState.ingredient_id || !formState.unit_id || !formState.quantity_needed) return;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Link ingredients to <span className="font-semibold">{item.name}</span> to calculate dynamic costs and automate production usage.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    recipeLines.map((line) => {
                      ...
                      const lineCost = ingredient?.cost_per_unit
                        ? ingredient.cost_per_unit * (line.quantity_needed || 1)
                        : ingredient?.calculated_cost_per_unit
                        ? ingredient.calculated_cost_per_unit * (line.quantity_needed || 1)
                        : null;
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Add Ingredient</h3>
            <div className="grid grid-cols-1 sm:grid-cols-[2fr_1fr_1fr_auto] gap-3 items-end">
              <div>
                <label className="text-xs font-medium text-muted-foreground block mb-1">Ingredient Item</label>
                <Select
                  value={formState.ingredient_id}
                  onValueChange={(value) => setFormState((prev) => ({ ...prev, ingredient_id: value }))}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormState((prev) => ({ ...prev, quantity_needed: e.target.value }))}
                />
              </div>
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormState((prev) => ({ ...prev, unit_id: value }))}
                >
                  <SelectTrigger>
          ...
          <div className="rounded-md border p-4 bg-muted/30 text-xs text-muted-foreground space-y-1">
            ...
            <p>Recipe-calculated cost: {formatCurrency(item.recipe_cost_per_unit ?? item.calculated_cost_per_unit)}</p>
            {item.recipe_yield_quantity && (
              <p>
                Yield: {item.recipe_yield_quantity} {item.recipe_yield_unit?.abbreviation || item.recipe_yield_unit?.name || item.unit?.abbreviation || item.unit?.name}
              </p>
            )}
          </div>
        </div>
