...
import type { InventoryItemUnit } from "@/features/inventory/hooks/types";
...
export function EnhancedCountingTable({
  countId,
  onCountUpdate,
}: EnhancedCountingTableProps) {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesUncounted =
      !showUncountedOnly || !counts[item.id]?.is_completed;
  ...
  const calculateVariance = (
    ...
    const countedInBaseUnits = Object.entries(itemCounts.unit_counts).reduce(
      (total, [unitId, quantity]) => {
        const unit = item.units.find((u: InventoryItemUnit) => u.id === unitId);
        if (unit) {
  ...
  const getVarianceStatus = (variance: number) => {
    ...
    if (absVariance === 0)
      return { status: "accurate", color: "bg-green-100 text-green-800" };
    if (absVariance <= 5)
      return { status: "minor", color: "bg-yellow-100 text-yellow-800" };
    return { status: "significant", color: "bg-red-100 text-red-800" };
  };

  if (isLoading) {
    return (
      <div className="flex justify-center p-8">Loading inventory items...</div>
    );
  }
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold">
              {filteredItems.length - savedItems.size}
            </div>
