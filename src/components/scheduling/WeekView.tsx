...
export function WeekView({
  ...
  const getCoverageStats = useMemo(
    () => (day: Date) => {
  ...
  const eventsByShiftId = useMemo(() => {
    ...
    overlayEvents.forEach((event) => {
      const fallback = Array.isArray(event.raw?.related_shift_ids)
        ? event.raw.related_shift_ids
        : [];
          ...
          <div className="flex space-x-2 pb-2" style={{ minWidth: "700px" }}>
            {weekDays.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className="flex-1 min-w-[90px] p-3 border rounded-lg bg-white"
                >
    ...
    <div className="h-full">
      ...
      <div className="grid grid-cols-8 border-b bg-muted/40">
        <div className="p-3 border-r">
          ...
          <div className="text-[11px] text-muted-foreground">
            Sessions linked to the schedule
          </div>
        </div>
        {weekDays.map((day) => {
              ...
              <div className="grid grid-cols-8">
                {employees.map((employee) => (
                  <Fragment key={employee.id}>
                    ...
                    {weekDays.map((day) => {
                      const employeeShifts = getShiftsForDay(day).filter(
                        (shift) => {
                          ...
                          return shift.assignments?.some(
                            (assignment: AssignmentWithUser) =>
                              assignment.user &&
                              (assignment.user.id === employee.id ||
                                `${assignment.user.first_name}-${assignment.user.last_name}` ===
                                  employee.id),
                          );
                        },
                      ...
                      return (
                        <div
                          key={`cell-${day.toISOString()}-${employee.id}`}
