...
export function AIInsightsDashboard() {
  ...
  const loadInsights = async () => {
    try {
      ...
      // Get AI insights
      const { data: insightsData, error: insightsError } = await supabase.functions.invoke('ai-scheduling-assistant', {
        body: { 
      ...
      const { data: coverageData, error: coverageError } = await supabase.functions.invoke('ai-scheduling-assistant', {
        body: { 
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">AI Insights Dashboard</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-6">
            ...
            <div className="text-2xl font-bold">{coverage?.coveragePercentage || 0}%</div>
            <Progress value={coverage?.coveragePercentage || 0} className="mt-2" />
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="text-2xl font-bold">{coverage?.totalHours || 0}h</div>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="text-2xl font-bold">${coverage?.estimatedCost || 0}</div>
            <p className="text-xs text-muted-foreground mt-1">
              Est. weekly cost
            </p>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="text-2xl font-bold">{coverage?.efficiency || 0}%</div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {insights?.overworkedStaff?.length > 0 && (
              <div>
                <h4 className="font-medium text-sm mb-2 text-red-600">Overworked Staff</h4>
                  ...
                  {insights.overworkedStaff.slice(0, 3).map((staff, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-red-50 dark:bg-red-950/20 rounded">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-medium">{staff.hours}h</div>
                        <div className="text-xs text-muted-foreground">{staff.recommendation}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {insights?.underutilizedStaff?.length > 0 && (
              <div>
                <h4 className="font-medium text-sm mb-2 text-blue-600">Underutilized Staff</h4>
                <div className="space-y-2">
                  {insights.underutilizedStaff.slice(0, 3).map((staff, index) => (
            ...
            <div className="space-y-3">
              ...
              {(!insights?.recommendations || insights.recommendations.length === 0) && (
                <div className="text-center py-6 text-muted-foreground">
                  ...
                  <p className="text-xs">AI is analyzing your scheduling patterns</p>
                </div>
              )}
            </div>
          </CardContent>
