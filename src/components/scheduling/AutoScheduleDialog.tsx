...
export function AutoScheduleDialog({ open, onOpenChange, defaultLocationId }: AutoScheduleDialogProps) {
  const availableRuleSets = useMemo(() => describeAvailableRuleSets(), []);
  const [locationId, setLocationId] = useState<string>(defaultLocationId ?? availableRuleSets[0]?.id ?? 'inman-sq');
  ...
  const selectedRuleSet = availableRuleSets.find((rule) => rule.id === locationId);
