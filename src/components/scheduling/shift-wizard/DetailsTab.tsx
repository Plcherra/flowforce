...
export function DetailsTab({
      ...
      try {
        ...
        const companyId =
          typeof metadata["company_id"] === "string"
            ? (metadata["company_id"] as string)
            : null;
        let query = supabase
          .from("shift_templates")
          .select(
            "id, name, description, duration_hours, is_all_day, job_position_id, required_headcount, default_notes, color",
          )
          .order("name");
        ...
        logger.error("Failed to load shift templates", {
          error: err,
          tags: ["error"],
        });
      }
  ...
  const filteredPositions = useMemo(
    () =>
      positions.filter((position) =>
        position.name
          .toLowerCase()
          .includes((formData.job_position_input || "").toLowerCase()),
      ),
  ...
  const filteredTemplates = useMemo(
    () =>
      shiftTemplates.filter((tpl) =>
        tpl.name?.toLowerCase().includes(templateInput.toLowerCase()),
      ),
  ...
  const applyTemplate = (template: ShiftTemplate) => {
    ...
    setFormData((prev) => ({
      ...
      job_position_input: template.job_position_id
        ? (positions.find((p) => p.id === template.job_position_id)?.name ??
          prev.job_position_input)
        : prev.job_position_input,
      required_headcount:
        template.required_headcount || prev.required_headcount,
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          <Input
            ...
            onFocus={() =>
              setShowPositionSuggestions(Boolean(formData.job_position_input))
            }
            onBlur={() =>
              setTimeout(() => setShowPositionSuggestions(false), 120)
            }
            onKeyDown={(event) => {
              ...
              if (event.key === "ArrowDown") {
                ...
                setPositionActiveIndex((prev) =>
                  Math.min(prev + 1, filteredPositions.length - 1),
                );
                ...
                const pick =
                  filteredPositions[Math.max(positionActiveIndex, 0)];
                if (pick) {
                  setFormData((prev) => ({
                    ...prev,
              ...
              <div className="max-h-56 overflow-y-auto py-1">
                ...
                {filteredPositions.length === 0 && (
                  <div className="px-3 py-2 text-sm text-muted-foreground">
                    No matches. Press Enter to keep as typed.
                  </div>
                )}
              </div>
            </div>
        ...
        {formData.job_position_input && !formData.job_position_id && (
          <p className="text-xs text-muted-foreground">
            Will save without linking to a saved position unless you choose a
            suggestion.
          </p>
        )}
      </div>
          ...
          <CardContent className="pt-4">
            <div className="flex items-center space-x-4 text-sm">
              ...
              {formData.breaks.some((item) => !item.is_paid) && (
                <div className="flex items-center space-x-1">
                  <span>
                    {formData.breaks.filter((item) => !item.is_paid).length}{" "}
                    Unpaid break
                  </span>
                  <span>
                    {formatBreakTime(
        ...
        <div className="relative flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              ...
              onBlur={() =>
                setTimeout(() => setShowTemplateSuggestions(false), 120)
              }
              onKeyDown={(event) => {
                ...
                if (event.key === "ArrowDown") {
                  ...
                  const pick =
                    filteredTemplates[Math.max(templateActiveIndex, 0)];
                  if (pick) {
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          <Input
            ...
            onFocus={() =>
              setShowLocationSuggestions(Boolean(formData.location))
            }
            onBlur={() =>
            ...
            <div className="absolute z-50 mt-1 w-full rounded-md border bg-popover text-popover-foreground shadow-md">
              <div className="max-h-56 overflow-y-auto py-1">
                ...
                {distinctLocations.filter((location) =>
                  ...
                  <div className="px-3 py-2 text-sm text-muted-foreground">
                    No saved locations. Type to add a new one.
                  </div>
                )}
              </div>
            </div>
