...
export function TeamAvailabilityPanel({ className }: TeamAvailabilityPanelProps) {
  const { toast } = useToast();
  const { user } = useAuth();
  const { profile, loading: profileLoading } = useProfile();
  const { loading: schedulingLoading, refetchAll, isFallbackData } = useScheduling();
  ...
  const baseQueriesEnabled = canManageAvailability && hasOrgContext && !profileLoading;
  ...
  const missingAvailabilityRequest = queriesEnabled && isMissingRelationError(requestsError, 'availability_request');
  const missingAvailabilityTable = queriesEnabled && isMissingRelationError(exceptionsError, 'availability_exception');
  const missingStaffAvailability = queriesEnabled && isMissingRelationError(exceptionsError, 'staff_availability');
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>Connect your Supabase tables to unlock organization-wide availability management.</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="border-primary/40 bg-primary/5 text-primary">
            ...
            <AlertDescription>
              Managers will be able to manage locks, approve requests, and handle exceptions once live scheduling data
              is available.
            </AlertDescription>
          </Alert>
        </CardContent>
      ...
      <Card className={className}>
        <CardContent className="py-10">
          <Alert variant="destructive" className="border-destructive/40">
            ...
            <AlertDescription>
              We couldn&apos;t load your profile details. Refresh the page or contact an administrator if the issue
              persists.
            </AlertDescription>
          </Alert>
        </CardContent>
      ...
      <Card className={className}>
        <CardContent className="py-10">
          <Alert className="border-primary/40">
            ...
            <AlertDescription>
              ConnectFlow needs an active company to manage availability. Add your company profile or reach out to
              support for help.
            </AlertDescription>
          </Alert>
        </CardContent>
      ...
      <Card className={className}>
        ...
        <CardContent>
          <Alert className="border-primary/40">
            ...
            <AlertDescription>
              Only managers or owners can control team availability. Ask your administrator for the right permissions if
  ...
  if (missingOrgPrefs) {
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle>Team Availability Controls</CardTitle>
          <CardDescription>Initialize organization preferences to start managing availability.</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="border-primary/40">
            <ShieldAlert className="h-5 w-5 text-primary" />
  ...
  if (missingAvailabilityRequest || missingAvailabilityTable || missingStaffAvailability) {
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle>Team Availability Controls</CardTitle>
          <CardDescription>Provision the availability tables to unlock approval workflows.</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="border-primary/40">
            <ShieldAlert className="h-5 w-5 text-primary" />
            ...
            <AlertDescription className="text-sm text-primary/90">
              Make sure migrations for <code>availability_request</code>, <code>availability_exception</code>, and{' '}
              <code>staff_availability</code> are applied before managing team availability.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
      ...
      <Card className={className}>
        <CardContent className="py-10">
          <Alert variant="destructive" className="border-destructive/40">
            ...
            <AlertDescription>
              {firstError.message || 'We ran into an unexpected error while loading availability data.'}
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle>Team Availability Controls</CardTitle>
        <CardDescription>Manage locks, approve requests, and handle exceptions for all locations.</CardDescription>
      </CardHeader>
