...
export function EmployeeDrawer({ employee, open, initialTab = 'profile', onOpenChange }: EmployeeDrawerProps) {
  ...
  const tabs = useMemo<DrawerTabConfig[]>(() => {
    const baseTabs: DrawerTabConfig[] = [
      ...
      { value: 'access', label: 'Access', enabled: hasEmployee && canManageAccess },
      { value: 'invite', label: 'Invitations', enabled: canInvite },
      { value: 'roles', label: 'Roles & Permissions', enabled: hasEmployee && canManageRoles },
    ];
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-hidden px-6 py-4 drawer-body">
            <Tabs value={activeTab} onValueChange={handleTabChange} className="flex h-full flex-col">
              ...
              {hasEmployee && (
                <TabsContent value="profile" className="mt-0 flex-1 overflow-y-auto">
                  <UserOverviewTab user={employee} />
                </TabsContent>
              )}

              {hasEmployee && canManageAccess && (
                <TabsContent value="access" className="mt-0 flex-1 overflow-y-auto">
                  <AccessControlPanel employeeId={employee.id} employee={employee} />
                </TabsContent>
              )}

              {canInvite && (
                <TabsContent value="invite" className="mt-0 flex-1 overflow-y-auto">
                  <InviteEmployeePanel />
                </TabsContent>
              )}

              {hasEmployee && canManageRoles && (
                <TabsContent value="roles" className="mt-0 flex-1 overflow-y-auto">
                  <RoleConfigPanel employeeId={employee.id} employee={employee} />
                </TabsContent>
              )}
            </Tabs>
          </div>
...
function HeaderContent({ employee }: { employee: Profile | null }) {
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Invite teammates or resend pending invitations without leaving the directory.
          </p>
        </div>
      </div>
    );
  ...
  const initials = `${employee.first_name?.[0] ?? ''}${employee.last_name?.[0] ?? ''}`.toUpperCase() || 'TM';
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            {employee.employment_status && (
              <Badge variant={employee.employment_status === 'active' ? 'default' : 'outline'} className="capitalize">
                {employee.employment_status}
              </Badge>
            )}
          ...
          <p className="mt-2 break-all text-sm text-muted-foreground">{employee.email}</p>
        </div>
      </div>
    </div>
  );
