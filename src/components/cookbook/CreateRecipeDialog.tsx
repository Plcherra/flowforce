...
export function CreateRecipeDialog({
  ...
  const [ingredients, setIngredients] = useState<IngredientRow[]>([{ ingredientId: '', quantity: 0 }]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const validIngredients = ingredients.filter((row) => row.ingredientId && row.quantity > 0);
    if (!validIngredients.length) {
      toast({
        title: 'Add ingredients',
        description: 'Recipes require at least one ingredient linked to an inventory item.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: 'The recipe is ready to sync with Supabase once migrations run.',
      });
      resetForm();
      onOpenChange(false);
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={(value) => {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Link an Items &amp; Setup product with its ingredient inventory items for live costing.
          </DialogDescription>
        </DialogHeader>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline">
                  {inventoryItems.find((item) => item.id === selectedItemId)?.unit?.abbreviation || 'unit'}
                </Badge>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {ingredients.map((row, index) => {
                const ingredient = inventoryItems.find((item) => item.id === row.ingredientId);
                return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) => setNutrition((prev) => ({ ...prev, calories: event.target.value }))}
              />
              <div className="grid grid-cols-3 gap-2">
                <Input
                  ...
                  onChange={(event) => setNutrition((prev) => ({ ...prev, protein: event.target.value }))}
                />
                <Input
                  type="number"
                  placeholder="Carbs g"
                  value={nutrition.carbs || ''}
                  onChange={(event) => setNutrition((prev) => ({ ...prev, carbs: event.target.value }))}
                />
                <Input
                  type="number"
                  placeholder="Fat g"
                  value={nutrition.fat || ''}
                  onChange={(event) => setNutrition((prev) => ({ ...prev, fat: event.target.value }))}
                />
              </div>
              <p className="text-xs text-muted-foreground">
                Nutrition values sync to the recipe detail view and export sheets.
              </p>
            </div>
          </div>
