...
import { CustomSection as TemplateSection, BusinessTemplate } from '@/types/templates';
...
export default function SectionManager({ 
  ...
  const [categoryFilter, setCategoryFilter] = useState<'all' | 'core' | 'industry' | 'custom' | 'operations'>('all');
  const [sections, setSections] = useState<TemplateSection[]>(AVAILABLE_SECTIONS);
    ...
    if (businessTemplate) {
      filteredSections = AVAILABLE_SECTIONS.filter(section => {
        ...
        if (section.category === 'industry' || section.category === 'custom') {
          ...
          if (section.templateId && section.templateId !== businessTemplate.id) return false;
  ...
  const allSections = [...sections, ...customSections.map(cs => ({
    id: cs.id,
    name: cs.name,
      ...
      <div className="space-y-6">
        ...
        {customSections.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-semibold text-gray-900">Custom Sections</h4>
            ...
            <div className="grid gap-4">
              {customSections
                .filter(section => 
                  ...
                  (categoryFilter === 'all' || categoryFilter === 'custom' || section.category === categoryFilter)
                )
                .map(section => (
                  <CustomSectionCard
                    key={section.id}
        ...
        {Object.entries(groupedSections)
          .filter(([category]) => category !== 'custom' || customSections.length === 0)
          .map(([category, categorySections]) => (
            <SectionCategoryGroup
              ...
              onEditSection={can('systemSettings') ? handleEditSection : undefined}
              onDeleteSection={can('systemSettings') ? handleDeleteSection : undefined}
              onAddSection={can('systemSettings') ? handleCreateCustom : undefined}
              onAddFromTemplate={can('systemSettings') ? handleAddFromTemplate : undefined}
