...
export function UnifiedSectionsManager({ selectedSections, onSectionToggle }: Props) {
  const { sections: customSections } = useCustomSections();
  const [query, setQuery] = useState('');
  const [category, setCategory] = useState<'all' | 'core' | 'communication' | 'operations' | 'hr' | 'custom'>('all');
  ...
  const filtered = useMemo(() => {
    return items.filter((it) => {
      ...
      if (query.trim()) {
        ...
        if (!(`${it.name} ${it.path} ${it.description || ''}`.toLowerCase().includes(q))) return false;
      }
      return true;
    });
    ...
    <Card>
      <CardHeader className="pb-4">
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => navigate('/add-section')}>Add New Section</Button>
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        {filtered.map((it) => {
          ...
          return (
            <div key={`${it.type}-${it.id}`} className="flex items-center justify-between p-3 border rounded-lg">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium truncate">{it.name}</span>
                  <Badge className={catClass[it.category] || catClass.core}>{String(it.category)}</Badge>
                  <Badge variant="outline" className="text-xs">{it.type}</Badge>
                </div>
                <div className="text-xs text-muted-foreground mt-1 truncate">{it.path}</div>
              </div>
              <div className="flex items-center gap-2">
                <Switch checked={enabled} onCheckedChange={(v) => onSectionToggle(it.id, !!v)} />
              </div>
            </div>
          );
        })}
