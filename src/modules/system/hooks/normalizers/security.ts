...
export const normalizeSecurity = (value: unknown): SecuritySettings => {
  ...
  const tokenSource = isRecord(source.apiTokenAccess) ? source.apiTokenAccess : {};
  const rlsSource = isRecord(source.rowLevelSecurity) ? source.rowLevelSecurity : {};
  ...
  return {
    ...
    apiTokenAccess: {
      enabled: asBoolean(tokenSource.enabled, DEFAULT_SECURITY.apiTokenAccess.enabled),
    ...
    rowLevelSecurity: {
      enforced: asBoolean(rlsSource.enforced, DEFAULT_SECURITY.rowLevelSecurity.enforced),
