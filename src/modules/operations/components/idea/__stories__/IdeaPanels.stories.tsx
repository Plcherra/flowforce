...
const sampleInsights: IdeaKpiInsight[] = [
  { id: 'sales', label: 'Net Sales', value: 152000, delta: 6200, trend: 'up', unit: 'USD' },
  { id: 'waste', label: 'Waste %', value: 3.2, delta: -0.5, trend: 'down', unit: '%' },
  { id: 'labor', label: 'Labor %', value: 27.1, delta: 1.4, trend: 'up', unit: '%' },
];

const baseContext: IdeaContextValue = {
  stage: 'identify',
  setStage: () => void 0,
  range: { start: new Date('2024-01-01T00:00:00Z'), end: new Date('2024-01-08T00:00:00Z') },
  setRange: () => void 0,
  companyId: undefined,
  activeCycleId: null,
...
const diagnosticsMock = {
  data: {
    causes: [{ id: 'c1', summary: 'Rising labor cost on weekends', confidence: 0.84 }],
    recommendations: [{ id: 'r1', action: 'Rebalance weekend staffing', impact: 'Reduce overtime by 15%', confidence: 0.76 }],
  },
  loading: false,
  error: null,
  refresh: () => void 0,
...
const assessmentsMock = {
  data: [
    { metric: 'Net Sales', before: 120000, after: 152000, delta: 32000, roi: 26.7, unit: 'USD' },
  ],
  loading: false,
  error: null,
  refresh: () => void 0,
...
const StoryStageProvider = ({ stage, children }: { stage: IdeaContextValue['stage']; children: React.ReactNode }) => (
  <IdeaContext.Provider value={{ ...baseContext, stage }}>{children}</IdeaContext.Provider>
);
