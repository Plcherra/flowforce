...
import type {
  Schedule,
  ScheduleStatus,
} from "@/types/scheduling-unified";

/**
 * Transform raw database schedule to application Schedule type
 */
export function transformDbScheduleToSchedule(
  dbSchedule: Tables<"schedules">,
): Schedule {
  return {
    ...dbSchedule,
    ...
    requirements: Array.isArray(dbSchedule.requirements)
      ? dbSchedule.requirements
      : [],
 ...
 */
export function transformDbSchedulesToSchedules(
  dbSchedules: Tables<"schedules">[],
): Schedule[] {
...
export function calculateTotalHours(schedules: Schedule[]): number {
  return schedules.reduce((total, schedule) => {
    return (
      total + calculateShiftDuration(schedule.start_time, schedule.end_time)
    );
  }, 0);
}
