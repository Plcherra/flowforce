...
export const calculateCoverageStats = (shifts: ShiftWithAssignments[]): CoverageStats => {
  ...
  const totalHeadcount = shifts.reduce((sum, shift) => 
    sum + (shift.assignments?.length || 0), 0);
  
  const requiredHeadcount = shifts.reduce((sum, shift) => 
    sum + (shift.required_headcount || 1), 0);

  return {
    totalHours: Math.round(totalHours * 10) / 10,
    totalHeadcount,
    requiredHeadcount,
    coverageRatio: requiredHeadcount > 0 ? totalHeadcount / requiredHeadcount : 1
  };
};
