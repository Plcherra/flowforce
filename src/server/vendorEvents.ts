...
export async function linkToShift(eventId: string, shiftId: string | null) {
  ...
  logger.warn(
    "linkToShift: vendor_visits table does not support shift_id. Consider using linked_event_id instead",
    { tags: ["warning"] },
  );
  return {
    data: null,
    error: new Error("shift_id is not supported in vendor_visits schema"),
  };
}
