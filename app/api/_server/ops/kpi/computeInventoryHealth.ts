...
export async function computeInventoryHealth(
  ...
  const logger = createServerLogger("computeInventoryHealth", {
    orgId,
    tags: ["kpi"],
  });
  let healthyCount = 18;
  ...
  try {
    ...
    if (error) {
      ...
      healthyCount = data.filter((item) => {
        if (item.par_level == null || item.on_hand_quantity == null)
          return true;
        const ratio =
          Number(item.on_hand_quantity) / Number(item.par_level || 1);
