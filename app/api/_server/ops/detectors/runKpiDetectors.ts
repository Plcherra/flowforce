...
const DETECTORS = [
  computeTasksCompliance,
  computeInventoryHealth,
  computeLaborVsSales,
];

export async function runKpiDetectors({
  orgId,
}: RunKpiDetectorsOptions): Promise<OpsKpiSnapshot[]> {
  const logger = createServerLogger("runKpiDetectors", {
    orgId,
    tags: ["kpi", "detector"],
  });
  ...
  const snapshots = await Promise.all(
    DETECTORS.map((detector) => detector(orgId)),
  );
  ...
  if (error) {
    logger.error("Failed to upsert KPI snapshots", {
      error,
