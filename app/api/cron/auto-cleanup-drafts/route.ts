...
async function handle(request: NextRequest) {
  ...
  try {
    ...
    logger.info("Draft cleanup finished", {
      context: { deletedCount, cutoff: cutoffIso },
    });
...
async function deleteDrafts(
  ...
  logger.warn("Primary draft cleanup failed, attempting fallback", {
    error,
    context: { cutoff: cutoffIso },
  });
  ...
  if (fallback.error) {
    logger.error("Fallback draft cleanup failed", {
