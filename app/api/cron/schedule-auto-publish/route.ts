...
async function handle(request: NextRequest) {
  ...
  try {
    ...
    logger.info("Schedule auto-publish finished", {
    ...
    return NextResponse.json(
      { error: "schedule_auto_publish_failed" },
      { status: 500 },
    );
  }
}
...
async function publishEligibleSchedules(
  ...
  logger.warn(
    "Schedule publish failed with published_at column, retrying status-only",
    {
      error,
  ...
  if (fallback.error) {
    logger.error("Schedule publish fallback failed", {
