-- Additional performance optimizations (removing immutable function issues)
...
-- Add indexes for date range queries (without immutable function predicates)
CREATE INDEX IF NOT EXISTS idx_schedules_date_range ON public.schedules(start_time, end_time);
CREATE INDEX IF NOT EXISTS idx_expenses_date_range ON public.expenses(expense_date, created_at);
CREATE INDEX IF NOT EXISTS idx_payments_date_range ON public.payments(due_date, created_at);
...
-- Add safe partial indexes without immutable functions
CREATE INDEX IF NOT EXISTS idx_schedules_active_status ON public.schedules(user_id, start_time) WHERE status = 'scheduled';
CREATE INDEX IF NOT EXISTS idx_expenses_pending_status ON public.expenses(created_by, expense_date) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_payments_pending_status ON public.payments(created_by, due_date) WHERE status = 'pending';

-- Add unique constraint indexes that might be missing
CREATE UNIQUE INDEX IF NOT EXISTS idx_company_invites_email_company_pending ON public.company_invites(email, company_id) WHERE status = 'pending';
CREATE UNIQUE INDEX IF NOT EXISTS idx_profiles_employee_id_company_active ON public.profiles(employee_id, company_id) WHERE employee_id IS NOT NULL;