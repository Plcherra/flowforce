-- Performance Optimization: Strategic Index Creation (Transaction-Safe)
-- Add indexes for common query patterns and foreign keys
-- Using regular CREATE INDEX (not CONCURRENTLY) for transaction compatibility
...
CREATE INDEX IF NOT EXISTS idx_schedules_status_covering ON public.schedules(status, start_time, end_time) WHERE status IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_inventory_items_status_covering ON public.inventory_items(status, current_stock, min_stock_level) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_forms_status_covering ON public.forms(status, created_at, created_by) WHERE status = 'published';

-- Partial indexes for common filters
CREATE INDEX IF NOT EXISTS idx_schedules_published ON public.schedules(start_time, end_time) WHERE is_published = true;
CREATE INDEX IF NOT EXISTS idx_inventory_low_stock ON public.inventory_items(name, current_stock, min_stock_level) WHERE current_stock <= min_stock_level;
CREATE INDEX IF NOT EXISTS idx_reminders_pending ON public.reminders(remind_at, user_id) WHERE completed = false;
...
CREATE INDEX IF NOT EXISTS idx_profiles_company_role ON public.profiles(company_id, role) WHERE company_id IS NOT NULL;

-- Performance monitoring: Add index to track user sessions and activity
CREATE INDEX IF NOT EXISTS idx_profiles_email_active ON public.profiles(email) WHERE employment_status = 'active';